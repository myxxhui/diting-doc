...
**一个通过 Neuro-Symbolic MoE（混合专家）架构将量化信号与专家路由强制交集，并用动态凯利公式最大化复利效率的分布式智能交易系统**，在胜率 ≥ 85%、年化复利 ≥ 45%、回撤 < 12% 的"不可能三角"约束下运行。
...
flowchart TB
    ...
    subgraph L2[L2: 战略维度（产品设计轴线+开发与交付轴线）]
        L2A[00_维度关系总览]
        L2B[01_产品设计维度]
        L2C[02～09 产品设计]
        L2D[01_开发与交付流程维度]
...
| 维度 | 约束条件 | 实现路径 |
|------|---------|---------|
| **认知边界** | 胜率 ≥ 85% | MoE 架构 + 领域专家 + 可解释性 |
| **复利增长** | 年化复利 ≥ 45% | 多策略池 + Cash Drag Monitor |
...
### 战略维度结构（产品设计轴线 + 开发与交付轴线）

L2 以 [02_战略维度/README.md](./02_战略维度/README.md) 为准：**00_维度关系总览**、**产品设计轴线**（01 总纲 + 02～09）、**开发与交付轴线**（01_开发与交付流程维度）。

```mermaid
flowchart LR
    A[L2: 战略维度] --> B0[00_维度关系总览]
    A --> B1[01_产品设计维度]
    A --> B2[02～09 产品设计]
    A --> B3[01_开发与交付流程维度]
...
| 验收类别 | 关键指标 | 阈值 | 文档 |
|---------|---------|------|------|
| **MoE 验收** | 专家胜率 | ≥ 85% | [03_成功标识详细规约.md](./05_成功标识与验证/03_成功标识详细规约.md) |
| **MoE 验收** | 整体胜率 | ≥ 85%（连续 6 个月） | [03_成功标识详细规约.md](./05_成功标识与验证/03_成功标识详细规约.md) |
...
| **复利验收** | 回测 CAGR（年化复合增长率） | ≥ 45% | [03_成功标识详细规约.md](./05_成功标识与验证/03_成功标识详细规约.md) |
...
| [ADR-005](./06_追溯与审计/ADR/005-defensive-compound-mode.md) | 防御性复利模式 | Accepted | 年化收益 ≥ 30% 时触发防御模式（与 L1 不可能三角 45% 目标区分，见 ADR-005 内说明） |
