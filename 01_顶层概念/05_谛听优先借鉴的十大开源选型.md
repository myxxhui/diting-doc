...
| 序号 | 项目 | 借鉴/融合点 | 对应谛听层次 | 核心理由（简述） | 许可证（参考） |
|------|------|-------------|--------------|------------------|-----------------|
| 1 | **Vn.py** | Gateway 抽象、实盘对接模式 | Module F / Repo-I 执行层 | 文档明确「深度参考」；实盘稳定性是生存底线 | MIT（以官方为准；新增 integration_package 须做许可证审查） |
| 2 | **AkShare** | 国内数据统一接口 | 11_ 采集层、Module A、L2 知识库 | 国内数据唯一明确支柱；无数据则无法验证 | Apache-2.0 |
| 3 | **OpenBB** | 国际/宏观/基本面 + Provider 可插拔 | 11_ 采集层、Module A、L2、B 轨 | 宏观/基本面最佳连接器；与 AkShare 双支柱 | MIT |
| 4 | **TA-Lib** | 技术指标口径统一 | Module B、Scanner | 与业界口径一致；支撑可解释与性能 | BSD |
| 5 | **PyPortfolioOpt** | 组合优化与仓位分配 | Module D 判官 | 文档「直接引入」；实现仓位闭环 | Apache-2.0 |
| 6 | **Qlib** | AI 工作流、YAML 配置、数据/因子管道 | Module B 研发与回测 | 配置驱动、可追溯；与 MoE 量化侧契合 | MIT |
| 7 | **VectorBT** | 全市场粗筛与扫描性能 | Module B Scanner | 满足 &lt;30 分钟；粗筛+精算分工 | Apache-2.0 |
| 8 | **Alphalens** | 因子质检、防过拟合 tear sheet | Module B 策略池上线前 | 文档「第二道防线」；支撑胜率与认知边界 | Apache-2.0 |
| 9 | **Freqtrade** | Telegram/WebUI、熔断与确认、Dry-Run | Module H、10_ 运营治理 | 文档「像素级参考」；运维与风控触达 | GPL-3.0 |
| 10 | **Lean** | 四层解耦与仿真度标准 | Repo-I 引擎设计、回测规约 | 架构严谨与回测可信；长期标准参照 | Apache-2.0 |

**许可证说明**：上表为参考，以各项目官方为准；**新增 integration_package 须做许可证审查**，满足商业使用与合规要求。
...
| 序号 | 项目 | L3 设计文档与小节 | 部署与依赖（L3 锚点 / DNA） | L5 验收锚点 |
|------|------|-------------------|------------------------------|-------------|
| 1 | Vn.py | [Stage3-06 ModuleF设计](../03_原子目标与规约/Stage3_模块实践/06_ModuleF设计.md#design-stage3-06-integration-vnpy)、[Stage4-02 ModuleF执行网关接入设计](../03_原子目标与规约/Stage4_MoE与执行网关/02_ModuleF执行网关接入设计.md#design-stage4-02-vnpy) | [06_ModuleF 依赖与构建](../03_原子目标与规约/Stage3_模块实践/06_ModuleF设计.md#design-stage3-06-integration-vnpy)；dna_module_f.integration_packages | [l5-stage-stage3_06](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_06)、[l5-stage-stage4_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage4_02) |
| 2 | AkShare | [11_数据采集与输入层规约](../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md)、[Stage2-02 采集逻辑与Dockerfile设计](../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile设计.md#design-stage2-02-integration-akshare) | [Stage2-02 依赖与镜像构建](../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile设计.md#design-stage2-02-deps)；dna_stage2_02.integration_packages | [l5-stage-stage2_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_02) |
| 3 | OpenBB | 同上 [Stage2-02](../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile设计.md#design-stage2-02-integration-openbb) | 同上 | [l5-stage-stage2_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_02) |
| 4 | TA-Lib | [Stage3-02 ModuleB设计](../03_原子目标与规约/Stage3_模块实践/02_ModuleB设计.md#design-stage3-02-integration-talib) | [Stage3-02 依赖与构建](../03_原子目标与规约/Stage3_模块实践/02_ModuleB设计.md#design-stage3-02-deps)；dna_module_b.integration_packages | [l5-stage-stage3_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_02) |
| 5 | PyPortfolioOpt | [Stage3-04 ModuleD设计](../03_原子目标与规约/Stage3_模块实践/04_ModuleD设计.md#design-stage3-04-integration-pfo) | [04_ModuleD 依赖与构建](../03_原子目标与规约/Stage3_模块实践/04_ModuleD设计.md#design-stage3-04-integration-pfo)；dna_module_d.integration_packages | [l5-stage-stage3_04](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_04) |
| 6 | Qlib | [Stage3-02 ModuleB设计](../03_原子目标与规约/Stage3_模块实践/02_ModuleB设计.md#design-stage3-02-integration-qlib) | 同上 Stage3-02 | [l5-stage-stage3_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_02) |
| 7 | VectorBT | [Stage3-02 ModuleB设计](../03_原子目标与规约/Stage3_模块实践/02_ModuleB设计.md#design-stage3-02-integration-vectorbt) | 同上 Stage3-02 | [l5-stage-stage3_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_02) |
| 8 | Alphalens | [Stage3-02 ModuleB设计](../03_原子目标与规约/Stage3_模块实践/02_ModuleB设计.md#design-stage3-02-integration-alphalens) | 同上 Stage3-02 | [l5-stage-stage3_02](../05_成功标识与验证/02_验收标准.md#l5-stage-stage3_02) |
| 9 | Freqtrade | Module H、10_ 运营治理；逻辑填充期可做最小接口，完整落地见运维阶段 | 运维阶段落地时须新增 L3+DNA+L4，见 10_ | — |
| 10 | Lean | [Stage4-03 回测或仿真验证设计](../03_原子目标与规约/Stage4_MoE与执行网关/03_回测或仿真验证设计.md#design-stage4-03-integration-lean)、[09_ 回测/仿真引擎职责与仿真度](../03_原子目标与规约/_共享规约/09_核心模块架构规约.md#回测仿真引擎职责与仿真度统一入口) | 架构参照，无运行时依赖 | [l5-stage-stage4_03](../05_成功标识与验证/02_验收标准.md#l5-stage-stage4_03) |

L5 汇总表见 [02_验收标准](../05_成功标识与验证/02_验收标准.md) 中 workflow_stages 表后「逻辑填充期接入点验收（与 L3 可追溯）」表。**部署与依赖**：各选型对应的「依赖与构建」要求见上表「部署与依赖」列；L4 步骤须在 Dockerfile/requirements 中显式安装并在镜像内验证，详见 L3 设计文档对应小节与 DNA `integration_packages`。
