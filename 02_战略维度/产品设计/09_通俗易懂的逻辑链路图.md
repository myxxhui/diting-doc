...
下图展示 **A 轨（现金奶牛轨）** 各可部署单元与模块的协作：从输入层到 Module A/B/C，再到判官 A 轨分支、风控盾与执行网关。详见 [09_ 核心模块架构规约](../../03_原子目标与规约/_共享规约/09_核心模块架构规约.md)。

```mermaid
flowchart TB
    subgraph 输入层["输入层：数据采集与存储"]
        INGEST[采集任务]
        INGEST --> L1[L1 TimescaleDB<br/>OHLCV]
        INGEST --> L2_A[L2 行业/营收<br/>industry_revenue_summary]
        INGEST --> L2_KG[L2 知识库<br/>Agri-KG / Tech-KG / Macro-KG]
    end

    subgraph ModuleA["Module A · 语义分类器"]
        A_IN[标的池 universe]
        A_IN --> A_CLASS[基于申万行业 + 营收占比分类]
        A_CLASS --> A_OUT[ClassifierOutput<br/>领域标签: 农业/科技/宏观/未知<br/>+ 置信度]
        L2_A --> A_CLASS
    end

    subgraph ModuleB["Module B · 量化扫描引擎"]
        B_IN[标的池 与 A 同批]
        B_IN --> B_SCAN[全 A 股向量化扫描<br/>趋势/反转/突破/板块强度]
        L1 --> B_SCAN
        B_SCAN --> B_OUT[QuantSignal<br/>technical_score, strategy_source<br/>sector_strength, pool_scores 等]
        B_OUT --> L2_QUANT[写入 L2 quant_signal_snapshot]
    end

    subgraph ModuleC_A["Module C · A 轨 MoE 议会"]
        C_IN[ClassifierOutput + QuantSignal]
        C_IN --> ROUTER[Router 按 Tag 分发]
        ROUTER --> R1{Tag?}
        R1 -->|农业| 农业[农业-Agent<br/>季节性/期货]
        R1 -->|科技| 科技[科技-Agent<br/>研发/大基金]
        R1 -->|宏观| 宏观[宏观-Agent<br/>大宗/汇率]
        R1 -->|未知| TRASH[Trash Bin]
        农业 --> C_OPINION[ExpertOpinion<br/>horizon=SHORT_TERM]
        科技 --> C_OPINION
        宏观 --> C_OPINION
        TRASH --> C_OPINION
        L2_KG -.-> 农业
    ...
    subgraph ModuleD_A["Module D · 判官（A 轨分支）"]
        D_IN[QuantSignal + ExpertOpinion 列表]
        D_IN --> D_HORIZON{horizon?}
        D_HORIZON -->|SHORT_TERM 或未设置| D_VOTE[投票：Quant Pass ∩ Expert BULLISH]
        D_VOTE --> D_QUANT{technical_score>70?}
        D_QUANT -->|否| D_PASS[ACTION_PASS]
        D_QUANT -->|是| D_EXPERT{至少一专家 is_supported 且 BULLISH?}
        D_EXPERT -->|否| D_PASS
        D_EXPERT -->|是| D_KELLY[动态凯利计算仓位]
        D_KELLY --> D_CASH[Cash Drag Monitor<br/>空仓>5天则放宽 B 阈值]
        D_CASH --> D_DEF[防御性复利检查<br/>年化≥30% 则仓位减半等]
        D_DEF --> D_VERDICT[Verdict: ACTION_BUY<br/>+ suggested_position_ratio]
    end

    subgraph ModuleE_A["Module E · 风控盾（仅 A 轨）"]
        E_IN[Verdict + 持仓/价格]
        E_IN --> E_PAY[盈亏比 ≥1.5?]
        E_PAY -->|否| E_REJECT[拒绝开仓]
        E_PAY -->|是| E_CORR[同 Domain 暴露≤上限?]
        E_CORR -->|否| E_REJECT
        E_CORR -->|是| E_HARD[持仓：单笔亏损>2%?]
        E_HARD -->|是| E_STOP[市价斩仓]
        E_HARD -->|否| E_PASS[通过 → 执行层]
    end

    subgraph ModuleF["Module F · 执行网关"]
        F_IN[风控通过的指令]
        F_IN --> F_TYPE{操作类型?}
        F_TYPE -->|买入/止盈| F_HITL[推送 HITL → 人工确认]
        F_HITL --> F_EXEC[确认后拆单执行]
        F_TYPE -->|止损| F_AUTO[机器自动市价单]
    end
...
下图展示 **B 轨（长期捕手轨）** 各模块的协作：B 轨候选写入、Router 仅送 VC-Agent、判官 B 轨分支（豁免 2% 与现金拖累）、逻辑证伪与大周期反转、以及 B 轨风控与执行。详见 [03_双轨制与VC-Agent](../../01_顶层概念/03_双轨制与VC-Agent.md)、[01_B轨系统设计](../../03_原子目标与规约/B轨/01_B轨系统设计.md)。

```mermaid
flowchart TB
    subgraph 输入层_B["输入层：数据采集与存储（B 轨相关）"]
        INGEST_B[采集任务]
        INGEST_B --> L1_B[L1 OHLCV]
        INGEST_B --> L2_FUND[L2 company_fundamental<br/>财报/营收增速/研发占比]
        INGEST_B --> L2_MGT[L2 management<br/>管理层与治理]
        INGEST_B --> L2_KG_B[L2 知识库<br/>产业/政策/宏观]
    end

    subgraph ModuleA_B["Module A · 语义分类器（B 轨可选扩展）"]
        A_B_READ[读取行业/营收]
        A_B_READ --> A_B_OUT[ClassifierOutput<br/>可选: b_track_eligible, lifecycle_stage]
        L2_FUND -.-> A_B_READ
    end

    subgraph ModuleB_B["Module B · B 轨预筛与候选"]
        B_B_SCAN[B 轨策略与打分<br/>long_term_score / 成长质量 / 研发强度等]
        B_B_SCAN --> B_B_PASS{B 轨通过?<br/>long_term_candidate=true}
        B_B_PASS -->|是| B_B_WRITE[写入 b_track_candidate_snapshot]
        B_B_PASS -->|否| B_B_SKIP[不进入 B 轨管线]
        L1_B --> B_B_SCAN
        L2_FUND --> B_B_SCAN
    end

    subgraph ModuleC_B["Module C · B 轨 MoE（仅 VC-Agent）"]
        C_B_ROUTER{long_term_candidate<br/>且 enable_vc_agent?}
        C_B_ROUTER -->|是| VC_AGENT[VC-Agent<br/>基本面 + 产业逻辑]
        C_B_ROUTER -->|否| C_B_END[不走 B 轨]
        VC_AGENT --> VC_READ[读取 b_track_candidate_snapshot<br/>+ company_fundamental / management]
        VC_READ --> VC_OPINION[ExpertOpinion<br/>horizon = LONG_TERM]
        VC_OPINION --> VC_CONSENSUS[写入 b_track_consensus<br/>key_assumptions, disproof_triggers,<br/>evidence_summary, time_horizon]
        L2_FUND --> VC_AGENT
        L2_MGT --> VC_AGENT
        L2_KG_B --> VC_AGENT
    end

    subgraph ModuleD_B["Module D · 判官（B 轨分支）"]
        D_B_IN[QuantSignal + ExpertOpinion<br/>horizon=LONG_TERM]
        D_B_IN --> D_B_HORIZON{horizon?}
        D_B_HORIZON -->|LONG_TERM| D_B_EXEMPT[豁免 2% 硬止损<br/>豁免现金拖累监控]
        D_B_EXEMPT --> D_B_VOTE[投票：Quant ∩ Expert]
        D_B_VOTE --> D_B_KELLY[动态凯利计算仓位]
        D_B_KELLY --> D_B_VERDICT[Verdict: ACTION_BUY<br/>B 轨仓位标识]
    end

    subgraph 持仓期_B["持仓期 · 逻辑证伪与大周期反转"]
        HOLD[B 轨持仓]
        HOLD --> EVENTS[事件流<br/>财报暴雷/管理层变动/政策颠覆]
        HOLD --> PRICE[标的·行业价格/均线/关键位]
        EVENTS --> DISPROOF[逻辑证伪模块<br/>比对 b_track_consensus.disproof_triggers]
        PRICE --> REVERSAL[大周期反转模块<br/>均线死叉/跌破关键支撑等]
        DISPROOF -->|命中| CUT_D[平仓指令<br/>原因码 logic_disproof]
        REVERSAL -->|命中| CUT_R[止盈/减仓指令<br/>原因码 major_trend_reversal]
    end

    subgraph ModuleE_B["Module E · 风控（B 轨分支）"]
        E_B_IN[B 轨 Verdict / 逻辑证伪·反转指令]
        E_B_IN --> E_B_NO_2[不施加 2% 硬止损<br/>不施加现金拖累]
        E_B_NO_2 --> E_B_LIMIT[B 轨专用约束<br/>单标的上限/敞口占比等]
        E_B_LIMIT --> E_B_PASS[通过 → 执行层]
        CUT_D --> E_B_IN
        CUT_R --> E_B_IN
    end

    subgraph ModuleF_B["Module F · 执行（B 轨仓位标识）"]
        F_B_IN[B 轨指令]
        F_B_IN --> F_B_TAG[订单/仓位带 B 轨标识]
        F_B_TAG --> F_B_HITL[买入/止盈: HITL 确认]
        F_B_TAG --> F_B_EXEC[执行；分账按 B 轨统计]
    end

    B_B_WRITE --> ModuleC_B
    L2_FUND --> ModuleA_B
    VC_OPINION --> ModuleD_B
    D_B_VERDICT --> ModuleE_B
    D_B_VERDICT --> HOLD
    E_B_PASS --> ModuleF_B
    E_B_PASS --> HOLD
```
...
flowchart TB
    ...
    subgraph L2知识库[L2 领域知识库：AI 推理用]
        农业[农业-KG<br/>农业周期、猪周期<br/>一号文件库]
        科技[科技-KG<br/>产业链图谱<br/>国产替代率]
