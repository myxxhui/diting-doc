...
**通俗解释**：
1. **数据采集**：就像看股票行情软件，收集所有股票的价格、成交量等信息
...
**通俗解释**：
- **认知边界（胜率 ≥ 80%）**：通过 DeepSeek-R1 的可解释推理、知识库的领域逻辑支撑、研产同构确保回测胜率延续到实盘
- **复利增长（年化 ≥ 30%）**：通过 VectorBT 向量化扫描多策略池并发、现金拖累监控提高资金利用率、成本治理确保 ROI 为正
...
所有维度最终都服务于一个目标：**在胜率 ≥ 80%、年化复利 ≥ 30%、回撤 < 12% 的约束下，稳定运行并持续盈利**。
