...
> [!WARNING] **本规约部分示例（Module A~F、Phase1/Phase2 等）基于 v1（旧 ABCDEF + Stage1~5）模型**
> 2026-05-13 第 3 批重构后，**当前阶段验收以 [02_验收标准.md](./02_验收标准.md)（22 行 workflow_stages）为准**。本文档保留为 DoD 通用思想与生产工业级 SLI/SLO/SLA 等示例。

> [!NOTE] **[TRACEBACK] 成功验证锚点**
> - **顶层概念**: [项目定义与核心价值](../01_顶层概念/01_项目定义与核心价值.md)
> - **顶层概念**: [战略目标与回报设计](../01_顶层概念/02_战略目标与回报设计.md)
> - **原子规约**: [00_四大模块抽象总纲](../03_原子目标与规约/00_六大模块抽象总纲.md)（v2 总纲）
> - **原子规约**: [运营治理与灾备规约](../03_原子目标与规约/_共享规约/10_运营治理与灾备规约.md)
> - **本文档**: L5 层级，DoD 通用思想与示例
...
|---------|------|------------|-------------|------------|
| **Agri-Agent** | 农业（猪周期、气候） | ≥ 85% | ≤ 65% | ≥ 15% |
| **Tech-Agent** | 硬科技（半导体、AI） | ≥ 85% | ≤ 65% | ≥ 15% |
| **Geo-Agent** | 宏观（大宗商品、汇率） | ≥ 85% | ≤ 65% | ≥ 15% |

#### 1.2 测量方法

```python
# diting-src/diting/validation/moe_win_rate.py
...
**核心原则**：系统整体胜率必须 ≥ 85%，这是"不可能三角"中"认知边界"的核心指标。

**验证方法**：

```python
# diting-src/diting/validation/overall_win_rate.py
...
| 指标 | 阈值 | 测量周期 | 验收标准 |
|------|------|---------|---------|
| **整体胜率** | ≥ 85% | 连续 6 个月 | 每月统计，连续 6 个月达标 |
...
```python
# diting-src/diting/validation/chaos_rejection_rate.py
...
```python
# diting-src/diting/validation/performance_validator.py
...
**利润锁定线定义**：
- 当年化收益达到 30% 时触发（与 L1 不可能三角 45% 目标区分，见 ADR-005）
- 触发后，胜率阈值从 85% 提至 90%，仓位减半
- 锁定 25% 利润垫
...
```python
# diting-src/diting/validation/compliance_validator.py
...
| **P1** | MoE 验收 | 胜率 ≥ 85% | 业务目标未达成 |
| **P1** | 复利验收 | CAGR ≥ 45% | 业务目标未达成 |
| **P2** | 工程验收 | 性能指标 | 用户体验下降 |
