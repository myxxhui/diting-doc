...
> [!WARNING] **本规约部分内容（追溯矩阵示例、Module A~F 引用、Phase1/Phase2 阶段名）基于 v1（旧 ABCDEF + Stage1~5）模型**
> 2026-05-13 第 3 批重构后，**主追溯请优先看新文档**：
> - [02_战略追溯矩阵.md](./02_战略追溯矩阵.md)（按新 5 pillar × 3 milestone）
> - [00_L2_L3_DNA_映射.md](./00_L2_L3_DNA_映射.md)（22 stages）
> - [01_L3_DNA_变更对L4影响表.md](./01_L3_DNA_变更对L4影响表.md)
> 本文档保留为通用追溯思想与示例（如 ADR 模板、链式哈希、责任矩阵），具体阶段映射以新文档为准。
...
> - **本文档**: L6 层级，定义追溯与审计的通用思想与生产工业级实现要求
...
```python
# diting-src/diting/audit/change_impact_analyzer.py
...
|------|---------|-----------|-----|---------|---------|---------|
| **diting-src** | v1.2.0 | abc123 | ADR-002 | config-v3 | data-v5 | deploy-v2 |
...
```python
# diting-src/diting/audit/version_tracer.py
class VersionTracer:
    """版本追溯器"""
...
```python
# diting-src/diting/audit/operation_auditor.py
...
```python
# diting-src/diting/audit/data_tracer.py
class DataTracer:
    """数据追溯器"""
...
```python
# diting-src/diting/audit/compliance_auditor.py
...
```python
# diting-src/diting/audit/consistency_checker.py
...
```python
# diting-src/diting/audit/report_generator.py
...
```python
# diting-src/diting/audit/traceability_api.py
