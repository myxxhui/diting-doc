...
> - **原子规约**: [架构设计共识与协作元规则](../03_原子目标与规约/03_架构设计共识与协作元规则.md)
> - **原子规约**: [运营治理与灾备规约](../03_原子目标与规约/10_运营治理与灾备规约.md)
...
## Context (背景)
需要同时满足"认知边界"（胜率 ≥ 80%）和"复利增长"（年化 ≥ 30%）的要求。单一模型无法同时满足这两个目标。

## Decision (决策)
采用 Neuro-Symbolic MoE（神经符号混合专家）架构：
- Router 根据 Domain Tag 动态选择专家
- 垂类专家（Agri/Tech/Geo）在其领域内胜率 ≥ 80%
- 专家意见与量化信号强制交集
...
|----------|--------|--------|--------|--------|--------|------|
| **认知边界**：胜率 ≥ 80% | 技术栈与架构 | 核心公式与MoE架构规约 | Phase1-ModuleC | 整体胜率 ≥ 80% | ADR-002 | ✅ |
| **认知边界**：胜率 ≥ 80% | 数据架构 | 数据版本控制规约 | Phase1-ModuleA | 可解释标的占比 ≥ 10% | ADR-002 | ✅ |
| **复利增长**：年化 ≥ 30% | 技术栈与架构 | 核心公式与MoE架构规约 | Phase1-ModuleB | CAGR ≥ 30% | ADR-002 | ✅ |
| **复利增长**：年化 ≥ 30% | 成本治理 | 动态配置中心规约 | Phase2-CashDrag | 现金拖累监控 | ADR-003 | ✅ |
...
| **ADR-002** | MoE 架构 | 不可能三角 | 技术栈与架构 | 核心公式与MoE架构规约 | Phase1-ModuleC | 专家胜率 ≥ 80% | ✅ |
