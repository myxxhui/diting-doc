...
> - **L4 实践**: [04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-exit)
> - **L5 锚点**: [02_验收标准#l5-stage-stage2_02](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_02)

<a id="design-stage2-02-goal"></a>
## 本步目标

OHLCV/新闻/行业 全数据结构与逻辑完整；Dockerfile 支持采集任务构建。**数据量与范围按生产级要求**：单标的日线历史深度 ≥5 年、标的覆盖与全 A 股 universe 一致、复权与回测/实盘一致；与 [06_A轨_生产级数据要求_设计](06_A轨_生产级数据要求_设计.md)、[11_ 生产级数据要求](../_共享规约/11_数据采集与输入层规约.md) 一致。

<a id="design-stage2-02-points"></a>
## 设计要点

- **数据源**：功能深度和广度完整；**数据量按生产级**：OHLCV 单标的日线 ≥5 年、标的范围 = 当前全 A 股（universe）、复权口径与 Stage4 回测/实盘对齐（见 [06_A轨_生产级数据要求_设计](06_A轨_生产级数据要求_设计.md)）。
- **任务**：ingest_ohlcv、ingest_industry_revenue、ingest_news（见 11_ 与 data_ingestion DNA）；**可选/扩展**：ingest_business_profile（标的主营业务构成 → L2 `symbol_business_profile`，见 [12_右脑数据支撑与Segment规约](../_共享规约/12_右脑数据支撑与Segment规约.md)）；**必须**在本阶段提供**全 A 股标的池（universe）**：universe 表及更新 Job，表名、字段、触发方式与 [11_ 全 A 股标的池](../_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe) 一致，供 `get_current_a_share_universe()` 读取。
- **写入**：L1 TimescaleDB、L2 知识库；遵循 DVC 版本化；universe 表落 L1/L2，供 `get_current_a_share_universe()` 读取。
...
| 类型 | 约定 | L4 对应 |
|------|------|---------|
| **准入条件** | Stage2-01 验证项 V1～V7 全部符合，含下游 `make verify-db-connection` 可过 | [02_A轨_采集逻辑与Dockerfile_实践#本步依赖检查与关键参数获取](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-deps-check) |
...
- **下游镜像推送与 Chart**：Stage2-04/06 约定采集镜像推送到项目 **ACR**（**make push-images**）；diting-infra **config** 中 **stack.ingest.image** 指向 ACR 且 **tag: latest**，Chart 使用最新镜像；拉取凭证 Secret 归属 **charts/diting-stack/manifests/**，见 [06_A轨_生产级数据要求_设计#design-stage2-06-ingest-image](06_A轨_生产级数据要求_设计.md#design-stage2-06-ingest-image)。
...
- **生产级 OHLCV**：ingest_ohlcv 拉取历史区间按 [06_A轨_生产级数据要求_设计](06_A轨_生产级数据要求_设计.md) 与 `global_const.yaml#data_ingestion.production.history_depth_years`（≥5 年）执行；复权口径与回测/实盘一致，实现与 Stage4 文档或配置对齐。
- **验收要点**：`make ingest-test`（或等价）至少覆盖 ingest_industry_revenue、ingest_ohlcv（A 股）、以及为 Module A 与 L2 供给的至少一条路径；数据量满足生产级（单标日线 ≥5 年、标的 = universe）；单测或集成测可 Mock AkShare 返回值，但接口与错误处理逻辑必须实现。
...
L4 实践文档中的 [功能实践项清单](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-feature-checklist) 须与本表一一对应；执行者按「功能项 ID」勾选实践状态。
...
| F8 | ingest-test 目标数据约定（生产级） | L4 [2.5 确认采集到的目标数据](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-target-data) | 约定满足生产级：单标日线 ≥5 年、标的 = universe 全 A 股、复权与 Stage4 一致；与 06_、11_ 一致 |
| F9 | 一键构建所有镜像 | [依赖与镜像构建](#design-stage2-02-deps) | make build-images（或约定名）存在且可执行；退出码 0 表示全部镜像构建成功 |
| F10 | 全 A 股标的池（universe）表 + 更新 Job | [11_ 全 A 股标的池](../_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe)、本设计「任务」 | universe 表与更新 Job 实现；diting-core 通过 get_current_a_share_universe() 读取；表名、字段、有效条件与 11_ 一致 |
| F11 | ingest_business_profile（可选）：标的主营业务构成 → L2 标的主营构成表、细分注册表初始化 | [12_右脑数据支撑与细分规约](../_共享规约/12_右脑数据支撑与Segment规约.md) | 采集逻辑写入标的主营构成表；细分标识仅用细分注册表中已注册值；表 DDL 见 12_ |

<a id="design-stage2-02-verification"></a>
### 验证与可执行验收（设计层定义）

L4 实践中的「示例与验证」须满足下列设计层验收定义；具体执行方式与命令见 [02_A轨_采集逻辑与Dockerfile_实践#示例与验证](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-examples-verify)。L4 [验证与测试结果清单](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-verify-checklist) 中的验证项 ID（V-DB、V-INGEST、V-DATA、V-IMAGE）与本表对应。
...
| **数据量、类型、内容符合预期** | 写入满足 L1 `ohlcv`、L2 `data_versions` 表结构与 07_ 版本化；**生产级**：单标日线 ≥5 年、标的覆盖与 universe 一致、复权与 06_/11_ 一致 | psql 对 ohlcv、data_versions、universe 的验证；与 [06_生产级数据要求](06_A轨_生产级数据要求_设计.md) 一致；可选 `make verify-data-production`（见 06_ 实践） | V-DATA |
...
3. **准出验证**：L4 实践文档 [验证与准出](../../04_阶段规划与实践/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md#l4-stage2-02-exit) 表中全部命令通过：`make verify-db-connection`、`make ingest-test`、**镜像内** `make ingest-test`；验收维度满足 [验证与可执行验收](#design-stage2-02-verification)。
