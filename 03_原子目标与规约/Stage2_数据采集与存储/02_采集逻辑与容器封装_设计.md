...
> - **05_ 对应项**: [01_顶层概念/05_当前阶段优先开源选型](../../02_战略维度/技术选型与调研/02_当前阶段优先开源选型.md) 第 2、3 项（AkShare、OpenBB）
...
> - **L4 实践**: [04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-exit)
...
| 类型 | 约定 | L4 对应 |
|------|------|---------|
| **准入条件** | Stage2-01 验证项 V1～V7 全部符合，含下游 `make verify-db-connection` 可过 | [02_采集逻辑与容器封装_实践#本步依赖检查与关键参数获取](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-deps-check) |
...
- **逻辑填充期借鉴组件**：本步实现依赖 [01_顶层概念/05_当前阶段优先开源选型](../../02_战略维度/技术选型与调研/02_当前阶段优先开源选型.md) 中 **AkShare**（国内数据）、**OpenBB**（国际/宏观/基本面）。
...
L4 实践文档中的 [功能实践项清单](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-feature-checklist) 须与本表一一对应；执行者按「功能项 ID」勾选实践状态。
...
| F8 | ingest-test 目标数据约定（生产级） | L4 [2.5 确认采集到的目标数据](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-target-data) | 约定满足生产级：单标日线 ≥5 年、标的 = universe 全 A 股、复权与 Stage4 一致；与 06_、11_ 一致 |
...
L4 实践中的「示例与验证」须满足下列设计层验收定义；具体执行方式与命令见 [02_采集逻辑与容器封装_实践#示例与验证](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-examples-verify)。L4 [验证与测试结果清单](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-verify-checklist) 中的验证项 ID（V-DB、V-INGEST、V-DATA、V-IMAGE）与本表对应。
...
3. **准出验证**：L4 实践文档 [验证与准出](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与容器封装_实践.md#l4-stage2-02-exit) 表中全部命令通过：`make verify-db-connection`、`make ingest-test`、**镜像内** `make ingest-test`；验收维度满足 [验证与可执行验收](#design-stage2-02-verification)。
