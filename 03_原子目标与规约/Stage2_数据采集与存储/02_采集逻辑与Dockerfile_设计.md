...
OHLCV/新闻/行业 全数据结构与逻辑完整；Dockerfile 支持采集任务构建。**数据量与范围按生产级要求**：单标的日线历史深度 ≥5 年、标的覆盖与全 A 股 universe 一致、复权与回测/实盘一致；与 [06_生产级数据要求_设计](06_生产级数据要求_设计.md)、[11_ 生产级数据要求](../_共享规约/11_数据采集与输入层规约.md) 一致。

<a id="design-stage2-02-points"></a>
## 设计要点

- **数据源**：功能深度和广度完整；**数据量按生产级**：OHLCV 单标的日线 ≥5 年、标的范围 = 当前全 A 股（universe）、复权口径与 Stage4 回测/实盘对齐（见 [06_生产级数据要求_设计](06_生产级数据要求_设计.md)）。
- **任务**：ingest_ohlcv、ingest_industry_revenue、ingest_news（见 11_ 与 data_ingestion DNA）；**必须**在本阶段提供**全 A 股标的池（universe）**：universe 表及更新 Job，表名、字段、触发方式与 [11_ 全 A 股标的池](../_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe) 一致，供 `get_current_a_share_universe()` 读取。
...
- **下游镜像推送与 Chart**：Stage2-04/06 约定采集镜像推送到项目 **ACR**（**make push-images**）；diting-infra **config** 中 **stack.ingest.image** 指向 ACR 且 **tag: latest**，Chart 使用最新镜像；拉取凭证 Secret 归属 **charts/diting-stack/manifests/**，见 [06_生产级数据要求_设计#design-stage2-06-ingest-image](06_生产级数据要求_设计.md#design-stage2-06-ingest-image)。
...
- **生产级 OHLCV**：ingest_ohlcv 拉取历史区间按 [06_生产级数据要求_设计](06_生产级数据要求_设计.md) 与 `global_const.yaml#data_ingestion.production.history_depth_years`（≥5 年）执行；复权口径与回测/实盘一致，实现与 Stage4 文档或配置对齐。
...
| F8 | ingest-test 目标数据约定（生产级） | L4 [2.5 确认采集到的目标数据](../../04_阶段规划与实践/Stage2_数据采集与存储/02_采集逻辑与Dockerfile实践.md#l4-stage2-02-target-data) | 约定满足生产级：单标日线 ≥5 年、标的 = universe 全 A 股、复权与 Stage4 一致；与 06_、11_ 一致 |
...
| **数据量、类型、内容符合预期** | 写入满足 L1 `ohlcv`、L2 `data_versions` 表结构与 07_ 版本化；**生产级**：单标日线 ≥5 年、标的覆盖与 universe 一致、复权与 06_/11_ 一致 | psql 对 ohlcv、data_versions、universe 的验证；与 [06_生产级数据要求](06_生产级数据要求_设计.md) 一致；可选 `make verify-data-production`（见 06_ 实践） | V-DATA |
