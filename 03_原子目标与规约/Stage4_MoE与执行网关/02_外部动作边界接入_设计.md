...
> - **05_ 对应项**: [01_顶层概念/05_当前阶段优先开源选型](../../02_战略维度/技术选型与调研/02_当前阶段优先开源选型.md) 第 1 项（Vn.py）
> - **战略维度**: [平台技术栈与系统架构](../../02_战略维度/平台与产品/02_平台技术栈与系统架构.md)、[外部系统接入与边界抽象](../../02_战略维度/平台与产品/08_外部系统接入与边界抽象.md)
...
> - **L4 实践**: [04_阶段规划与实践/Stage4_MoE与执行网关/02_外部动作边界接入_实践](../../04_阶段规划与实践/Stage4_MoE与执行网关/02_外部动作边界接入_实践.md#l4-stage4-02-exit)
...
- **BrokerDriver**：抽象与占位/仿真实现；**深度参考 [Vn.py](https://github.com/vnpy/vnpy) 的 Gateway 设计**（事件驱动、多柜台统一接口），与 [01_顶层概念/05_当前阶段优先开源选型](../../02_战略维度/技术选型与调研/02_当前阶段优先开源选型.md) 一致。
- **E→F**：风控通过后指令至执行网关
...
本小节与 [Stage3-06 Module F 设计](../Stage3_模块实践/04_决策风控与动作主链_设计.md#design-stage3-04-integration-vnpy) 中「逻辑填充期开源接入点：Vn.py」互补：Stage3-06 侧重接口形态与占位实现，本步侧重**实盘或仿真接入**与 E→F 验证。
...
- **依赖与构建**：执行网关镜像的依赖与 [Stage3-06 Module F 设计](../Stage3_模块实践/04_决策风控与动作主链_设计.md#design-stage3-04-integration-vnpy) 中「依赖与构建」一致，确保占位/仿真或实盘适配器在镜像内可运行。
