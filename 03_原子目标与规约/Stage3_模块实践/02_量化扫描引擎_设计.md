...
- **Scanner**：technical_score_threshold、sector_strength_threshold（见 core_modules/dna_module_b）；标的池以 get_current_a_share_universe() 返回为准
...
- **详细需求**：
  ...
  - **性能与依赖**：TA-Lib 的安装与构建见本节「[依赖与构建](#design-stage3-02-deps)」；若全市场（当前全 A 股）日级扫描，需约定是否批量调用、是否有缓存（如按日缓存指标结果）。
...
- **实践重点**：Scanner 的**全市场粗筛**（当前全 A 股 universe、技术面得分 &gt; 阈值）使用 VectorBT 的向量化回测与多参数扫描能力，在满足「全市场扫描 + 推理 &lt; 30 分钟」的前提下，把粗筛与 Module C/LLM 的「精算」分工固化下来。粗筛输出为「候选池」（标的列表 + 技术分数/机会丰度），精算只消费该候选池，不在此阶段对全市场做 LLM 调用。
- **详细需求**：
  ...
  - **性能约束**：全市场（当前全 A 股）日级扫描 + 后续推理总时长 &lt; 30 分钟；约定 VectorBT 的并行/分块策略（如按板块或标的批次）、以及所需资源（CPU/内存）的估算方式。
