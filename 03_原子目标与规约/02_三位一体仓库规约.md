...
│   ├── default.json        # 项目级 default（resource/env/deployment）
│   └── environments/
│       ├── dev/
│       │   └── deploy.json # dev 环境覆盖
│       └── prod/
│           └── deploy.json # prod 环境覆盖
├── observability/          # [可选] 监控配置（Prometheus/Grafana/Loki 等）
└── secrets/                # 当前阶段：Sealed-Secrets 加密 YAML 或 Helm 引用
```

配置须遵循 deploy-engine 的 `DeploymentConfig`（default / env / user_override 三层合并；`chart_path`、`values_files`、BaseResourceSpec、BaseEnvSpec 等）。参见 deploy-engine 的 `pkg/config/spec.go` 与文档。
