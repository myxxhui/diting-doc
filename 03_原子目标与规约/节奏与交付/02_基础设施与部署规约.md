...
本节约定 diting-core 及采集/模块镜像的运行时与依赖，与 [01_顶层概念/05_当前阶段优先开源选型](../../02_战略维度/技术选型与调研/02_当前阶段优先开源选型.md) 及 L3 各阶段设计「依赖与构建」、DNA `integration_packages` 一致。
...
| **C2 云与规格** | 云厂商、竞价实例与数据盘规格 **与 Stage1-03 一致或使用单独 tfvars**（如 `config/terraform-diting-prod.tfvars`）；与 [Stage1-03 基础设施 ECS 与 K3s](../../04_阶段规划与实践/Stage1_仓库与骨架/03_基础设施ECS与K3s_实践.md) 的 Terraform 配置保持一致或显式区分。 |
| **Up 产出** | K3s + TimescaleDB/L2/Redis（PVC 使用该数据盘）、NodePort + 公网 IP 暴露；连接信息输出至约定文件（如 `prod.conn`），供 [06_生产级数据要求_实践](../../04_阶段规划与实践/Stage2_数据采集与存储/06_生产级数据要求_实践.md) 及 01_/02_ 可选路线使用。 |
| **前半/后半** | **前半**：环境与数据继承（Up/Down/再次 Up 三项核心验证）；**后半**：采集逻辑生产级 + Up 脚本内调 `make ingest-test` 落库 + Down/再次 Up 后数据仍在。目标与验证概要见 [06_生产级数据要求_设计](../Stage2_数据采集与存储/06_生产级数据要求_设计.md#design-stage2-06-prod-data-env)。 |
