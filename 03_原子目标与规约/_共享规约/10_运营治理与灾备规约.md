...
与 L1 [02_战略目标与ROI](../../01_顶层概念/02_战略目标与ROI.md) 性能目标一致：**全市场扫描 + 推理 < 30 分钟**。

**超时或部分失败时的处理策略**（由本规约约定，L3 层须落地实践）：

| 场景 | 实现要求（须满足） | 可选策略 | 说明 |
|------|--------------------|----------|------|
| **整体超时**（扫描+推理 ≥ 30 分钟） | ① 不得阻塞下一周期；② 须告警并记录（时间、耗时、原因） | 跳过本轮 / 使用上一轮有效结果（须在 DNA 或配置中选定默认行为） | 阈值可配置，默认 30 分钟；实现须在 DNA 或运维手册写明所选策略 |
| **部分标的推理超时/失败** | ① 不得导致整轮无效；② 须记录该标的与原因并告警 | 跳过该标的 / 降级为 Simple Mode（须在配置中约定） | 与 [08_心跳协议与健康检查规约](./08_心跳协议与健康检查规约.md) 进程级超时协同 |
| **扫描阶段失败** | ① 须重试或明确跳过；② 重试次数上限由配置约定；③ 跳过时须告警 | 重试 N 次后跳过本轮（N 可配置） | 实现须在 DNA 或运维手册写明重试上限与跳过策略 |

**L3 实践验收**：实现方须在 L4 步骤或运维手册中写明上述三类的所选策略、配置项及默认值，并在可观测性中暴露「超时/失败触发次数」与「采用的动作」（跳过/降级/重试），便于审计与复盘。

### 生产就绪检查（Go-Live）

上线前须满足以下检查项，全部通过后方可进入 Level 3（Live）或生产发布。每项对应 L5 验收或 Runbook，实现与运维须可追溯。

| 检查项 | 说明 | L5 或 Runbook 引用 |
|--------|------|---------------------|
| **合规/密钥/通道** | 通道合规率 100%、密钥零泄露、程序化交易报备就绪 | L5 合规性验收、[02_验收标准](../../05_成功标识与验证/02_验收标准.md)；Runbook：密钥恢复、通道验证 |
| **DR 演练与 RTO 验证** | 至少完成 L1/L2 灾备演练，RTO 达标 | 本规约「B. 灾难恢复」§7 灾备演练；L5 工程性能验收 RTO < 5 分钟 |
| **可观测与告警** | 日志/指标/追踪就绪，关键告警规则部署 | [08_心跳协议与健康检查规约](./08_心跳协议与健康检查规约.md)；L5 l5-func-08 |
| **HITL 与通知可用性** | 买入/止盈确认通知可达，确认率可测 | [ADR-004](../../06_追溯与审计/ADR/004-human-in-the-loop.md)；Runbook：通知故障处理 |
| **Level 2 偏差 < 1%** | 仿真回放净值曲线与回测偏差度 < 1% | [03_架构设计共识](./03_架构设计共识与协作元规则.md) Level 2；L5 研产同构相关验收 |

**准出条件**：上述各项在 L5 [02_验收标准](../../05_成功标识与验证/02_验收标准.md) 或本规约对应小节中有明确验收或检查记录；上线审批前须逐项勾选并留存证据。

### SLO 与 SLA

以下为关键 SLO（服务级别目标）及未达标时的 SLA（服务级别协议）约定；与 Runbook、事件响应协同，便于运维与复盘。

| SLO 项 | 目标值 | 未达标时 SLA（约定） | Runbook/关联 |
|--------|--------|----------------------|--------------|
| **可用性** | ≥ 99.9% | 单月未达标：复盘根因、改进项入 backlog；连续两月未达标：升级至技术负责人决策 | 与故障分级 L1–L5 恢复策略一致 |
| **RTO（恢复时间目标）** | < 5 分钟（L2 故障） | 超时：按事件响应升级；事后必须做恢复演练复盘并更新 Runbook | [本规约 B. 灾难恢复](#b-灾难恢复-disaster-recovery)、L2 Runbook |
| **全市场扫描+推理** | < 30 分钟 | 超时按本规约「性能与可用性边界」执行跳过/降级；须告警并记录 | 本规约核心理念、可观测性 |
| **HITL 确认超时** | 5 分钟内确认/拒绝 | 超时后按 ADR-004 约定（拒绝或自动执行，由配置决定）；通知可用性未达标须复盘通知链路 | [ADR-004](../../06_追溯与审计/ADR/004-human-in-the-loop.md)、通知 Runbook |
| **Level 2 净值偏差** | < 1% | 偏差 ≥ 1% 触发自动回滚，不得进入 Level 3；见 [03_架构设计共识](./03_架构设计共识与协作元规则.md) | Level 2 回滚 Runbook |

**Runbook 挂钩**：故障分级与事件响应（本规约 §1.2）中已约定每级故障对应 Runbook；告警或 SLO 未达标触发处置时，须能关联到对应 Runbook 名称或路径（运维手册或 DNA 中维护）。

### 运维与人机交互

与 [ADR-004 Human-in-the-Loop](../../06_追溯与审计/ADR/004-human-in-the-loop.md) 及 L2 [01_产品设计维度](../../02_战略维度/产品设计/01_产品设计维度.md)#系统使用者与前端设计 一致；约定 HITL 确认载体、确认界面须展示字段与超时表现、各角色可访问范围。

| 项 | 约定 |
|----|------|
| **HITL 确认载体** | 买入/止盈确认须通过可靠通道送达确认人（如手机推送 + Telegram Bot 或 Web 确认页）；具体选型由实现与运维约定，须满足 5 分钟超时内可达。 |
| **确认界面须展示** | 标的、方向、数量、理由摘要（可选）、剩余有效时间；确认/拒绝按钮；超时后须明确提示「已超时」及后续处理（拒绝或按配置执行）。 |
| **各角色可访问范围** | 交易确认人：待确认列表与单笔确认；守夜人/风控：监控/告警与熔断操作；复盘查看者：只读绩效/分轨报表；管理员：配置与审计。权限与具体界面由实现约定，须与合规与审计要求一致。 |
| **与 Module F 衔接** | 确认结果回传至执行网关（Module F）的协议与状态机见 [09_核心模块架构规约](./09_核心模块架构规约.md) 及 [04_全链路通信协议矩阵](./04_全链路通信协议矩阵.md)；审计日志须记录每次确认/拒绝与超时。 |

### Freqtrade 与 Module H（守夜人）占位

与 [01_顶层概念/05_谛听优先借鉴的十大开源选型](../../01_顶层概念/05_谛听优先借鉴的十大开源选型.md) 第 9 项一致：**Freqtrade**（Telegram Bot、WebUI、Dry-Run、熔断与确认）为运维与风控触达的借鉴对象，逻辑填充期可做最小接口，**完整落地见运维阶段**。运维阶段落地时须**新增 L3 设计 + DNA integration_packages + L4 步骤**，与设计–DNA–实践 1:1:1 一致；Module H（守夜人）的依赖与部署届时在对应 L3/L4 中写明。
...
与合规性审计及运维事件响应（§1.2）区分：**安全事件**指密钥泄露、未授权访问、篡改、恶意注入等与机密与完整性相关的事件。

| 项 | 约定 |
|----|------|
| **安全事件级别** | **S1 严重**：密钥/凭证泄露或疑似泄露、未授权交易或配置变更；**S2 高**：异常登录、权限滥用、审计日志被篡改；**S3 中**：可疑 API 调用、异常流量。 |
| **报告路径** | 发现人 → 安全负责人/On-call → 必要时上报管理层与合规；S1 须在发现后 15 分钟内启动响应并记录。 |
| **处置原则** | S1：立即隔离受影响系统或账号、撤销/轮换疑似泄露密钥、暂停相关交易通道直至验证；S2：冻结账号、撤销权限、保留证据；S3：记录并调查、必要时限流或封禁。 |
| **事后复盘与文档化** | 每起 S1/S2 须有事后复盘：根因、影响范围、处置时间线、改进措施；结论写入安全事件库并可与 Runbook 关联，供后续演练与审计使用。 |
...
各故障级别对应的响应角色、升级时限与恢复判定如下；与上表一一对应。Runbook 名称或路径须在运维手册或 DNA 中约定，便于告警触发时直接引用。

| 故障级别 | 响应角色 / On-call | 升级时限与路径 | 恢复判定与沟通 |
|----------|--------------------|----------------|----------------|
| **L1** | 当值运维或开发（自动重启由系统执行） | 若 3 次重启仍失败，5 分钟内升级至 L2 响应人 | 进程恢复、健康检查通过；记录恢复时间 |
| **L2** | 运维 + 基础设施负责人 | 15 分钟内未恢复则升级至技术负责人 | 新节点拉起、数据恢复完成、RTO 验证 |
| **L3** | 技术负责人 + 风控 | 熔断生效后 30 分钟内通报；未恢复则升级 | 行情恢复、熔断解除、交易恢复前人工确认 |
| **L4** | 技术负责人 + DBA/基础设施 | 1 小时内未恢复升级至业务负责人 | 主从切换完成、数据一致性校验 |
| **L5** | 业务负责人 + 全员通报 | 按 BCP 跨区域切换；4 小时内决策 | 跨区域服务就绪、RPO/RTO 达标 |

**Runbook 挂钩**：每级故障对应的处置步骤须在 Runbook 中写明（如「L2_Runbook_竞价实例回收」）；告警规则或故障分级触发时，须能关联到对应 Runbook 名称或路径，便于值班人员执行。

#### 1.3 生产变更与值班

| 项 | 约定 |
|----|------|
| **生产变更窗口** | 非紧急变更应在约定时间窗内执行（如交易日收盘后或周末），具体窗口由运维手册或 DNA 约定；紧急热修可例外，须记录并事后复盘。 |
| **变更审批层级** | 生产配置/代码变更须经审批：低风险（如仅配置项）可由当值运维；涉及交易逻辑或通道的须技术负责人或更高；具体层级与流程由运维手册约定。 |
| **On-call 轮值** | 与 §1.2 事件响应对应：须有明确 On-call 轮值表（谁在何时值班）、联系方式和交接规范；L1–L5 升级路径中的「响应角色」即当值或升级对象。 |
| **升级矩阵** | 事件响应表（§1.2）中「升级时限与路径」即升级矩阵；可与值班表合并维护，确保任一时刻可找到当前 On-call 及上一级升级人。 |
...
在灾备演练（§7）基础上，对**容量规划**与**混沌演练**做规约化要求，便于生产前或周期内执行。

| 项 | 范围 | 频率 | 责任 |
|----|------|------|------|
| **容量规划** | 交易量/扫描 QPS、推理并发、存储与带宽预估；与 SLO（扫描耗时、RTO）对齐；扩容阈值与自动/手动策略 | 每季度评审；上线前必做一次 | 技术负责人 + 运维；结果写入 Runbook 或 DNA |
| **混沌演练** | 可选：随机杀 Pod、网络延迟/丢包、依赖不可用（如行情/LLM 超时）；与 L1–L2 故障场景互补，验证熔断与降级 | 每半年至少一次；可与 L2/L3 演练合并 | 运维或基础设施负责人；须在非交易时段或隔离环境执行，结果文档化 |

**说明**：容量规划确保上线与扩容有据可依；混沌演练验证系统在异常注入下的表现，与 09_ 错误处理与降级、本规约故障分级一致。具体脚本与环境由运维手册或 DNA 约定。
