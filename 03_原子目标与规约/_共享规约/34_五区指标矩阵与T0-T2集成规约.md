...
|-----------|--------|------|----------|----------|------|--------|------|---------------|----|----|-----|---|------|------|
| `M.policy.sector_direction` | 赛道政策方向 | **定制语料** | 段 A：**全市场** RSS；段 B 后 **按 `strategic_board` 关键词** 过滤 | 顺逆风 enum | S0→S1 | 发改委/工信部/国务院 | **滚动保留24个月** | 事件 · 日轮询 08:00 | RSS/公告入库 DeepSea | 去重 | **LLM 抽取 enum**（工程化见 [36_ §4](./36_Z0-M2政策赛道T1-语义工程化方案.md#design-36-t1-semantic)；启动期规则占位 `rule:z0_policy_t1_v1`） | V2 | `wind_scan` / board | E1/E3 |
