...
> - **对应 DNA**: `_System_DNA/core_modules/dna_module_*.yaml`（每模块独立）；成本治理（Token 预算、熔断）、production_requirements 见 `global_const`
...
flowchart TB
    ...
    subgraph ModuleA[Module A: 语义分类器]
        ...
        TAG[领域标签<br/>农业/科技/宏观]
    end
    
    subgraph ModuleB[Module B: 左脑 - 量化扫描引擎]
        SCANNER[Quant Scanner<br/>向量化扫描全 A 股（universe）]
        POOL1[Pool 1: 趋势策略]
        POOL2[Pool 2: 反转策略]
        POOL3[Pool 3: 突破策略]
    ...
    subgraph ModuleC[Module C: 右脑 - MoE 议会]
        ...
        农业[农业-Agent<br/>季节性/期货]
        科技[科技-Agent<br/>研发/大基金]
        宏观[宏观-Agent<br/>大宗/汇率]
...
| 输入 | 输出 |
|------|------|
| 标的代码（如 "000998.SZ"） | 领域标签列表（如 `[农业]`） |
| 申万行业分类 | 置信度分数（0.0-1.0） |
| 营收占比数据 | |
| **L2 标的主营构成表**（可选，见 [12_右脑数据支撑与细分规约](./12_右脑数据支撑与Segment规约.md)） | **主营占比列表**（segment_shares）：repeated { 细分标识、营收占比、是否主营 }，与主营构成表一致 |

**Proto**：输出对应 [ClassifierOutput](_Design_Artifacts/protocols/classifier/classifier_output.proto)；扩展字段 segment_shares 见 [04_ ClassifierOutput 扩展](./04_全链路通信协议矩阵.md)。

### 输出语义辨析（领域标签 vs 主营披露）

| 概念 | 数据来源 | 说明 |
|------|----------|------|
| **领域标签**（Proto 中 tags / L2 快照 `primary_tag`） | 申万一级行业（`industry_revenue_summary` + `industry_fallback`）与 `config/classifier_rules.yaml` | 分类主结果；申万仅「电力」等需细分时，可再按财报分部名做 YAML 映射（见 [12_](./12_右脑数据支撑与Segment规约.md)）。 |
| **主营披露**（`segment_shares` / L2 `symbol_business_profile`） | AkShare 主营构成 → L2 分部表 | 财报「按产品」披露，与申万行业**不是同一数据源**；无披露时占位 `segment_id=seg_no_disclosure`，快照 JSON 中 `disclosure_present=false`。 |

**标的池来源与同批一致**：Module A 的标的池 = **当前全 A 股（universe）**，由 [11_数据采集与输入层规约](./11_数据采集与输入层规约.md) 约定的 **`get_current_a_share_universe()`** 提供；该接口行为为「检查有效条件→按需更新→返回列表」。同一批次/同一日任务中，A 与 B 须使用**同一份**标的池，由调度/编排层**一次**调用 `get_current_a_share_universe()` 并将结果传入 A、B，或在同一执行流程入口处一次获取后分别传入。执行前通过该接口（或调用方传入）获取列表后，对**全部 N 只**全量分类，不写死数量。
...
class DomainTag(Enum):
    AGRI = 1      # 农业（猪周期、气候）；存储/展示用「农业」
    TECH = 2      # 硬科技（半导体、AI）；存储/展示用「科技」
    GEO = 3       # 宏观（大宗商品、汇率）；存储/展示用「宏观」
    UNKNOWN = 4   # 无法归类；存储/展示用「未知」
...
**目标**：**机会丰度 (Opportunity Abundance)**。每日对**当前全 A 股（universe）**进行向量化扫描，输出技术面得分 > 70 的候选池；标的池数量由运行时 `get_current_a_share_universe()` 发现，不写死。

**标的池来源与同批一致**：Module B 的标的池 = **当前全 A 股（universe）**，由 [11_数据采集与输入层规约](./11_数据采集与输入层规约.md) 约定的 **`get_current_a_share_universe()`** 提供；该接口行为为「检查有效条件→按需更新→返回列表」。同一批次/同一日任务中，B 与 A 须使用**同一份**标的池，由调度/编排层**一次**调用 `get_current_a_share_universe()` 并将结果传入 A、B，或在同一执行流程入口处一次获取后分别传入。执行前通过该接口（或调用方传入）获取列表后，对**全部 N 只**全量扫描。

**A 轨持仓与每日 B 的衔接**：B 只产出信号与风险字段（含 `a_track_short.risk`）；**持仓状态、止损合并、连续低于阈值日数**由执行侧按 [03_A轨_持仓与每日信号复核规约](../Stage3_模块实践/03_A轨_持仓与每日信号复核规约.md) 与 `core_modules/dna_a_track_position_lifecycle.yaml` 处理；纯函数实现见 `diting-core/diting/position_lifecycle.py`，供 Module E 调用。
...
- 可选：最近 3 根 K 线均满足两条件才给分（trend_confirm_bars）

**打分**：均线子分（0–50）=(满足 MA 不等关系条数/3)×50；MACD 子分（0–50）按柱状图归一化。两子分相加 cap 100。详见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md) §3.3；实现见 diting-core/diting/scanner/pools.py。

#### Pool 2: 反转策略 (Reversion)

**信号条件**：
- RSI(14) < 20（超卖，可配置放宽为 30）
- 价格触及 Bollinger 下轨（close ≤ lower×1.01）
- 可选止跌：close > MA5 才给分（reversion_require_above_ma5）

**打分**：RSI 子分（0–50）、布林下轨子分（0–50）连续计算，两子分相加 cap 100。详见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md) §3.3；实现见 diting-core/diting/scanner/pools.py。

#### Pool 3: 突破策略 (Breakout)

**信号条件**：
- 收盘价 > 前 20 日最高（不含当日）；成交量 > 2×SMA(volume,20)，量比 cap 5.0
- 可选：最近 2 根 K 线中至少 2 根满足双条件才给分（breakout_confirm_bars）

**打分**：价格突破子分（0–50）、放量子分（0–50）连续计算，两子分相加 cap 100。详见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md) §3.3；实现见 diting-core/diting/scanner/pools.py。

#### 可选：Pool 4 动量策略 (Momentum)

**开关**：`optimization.pool_4_momentum_enabled`。为 true 时增加第四池：20 日收益率分位 ≥ 0.80、close > MA20，两子分各 0–50，相加 cap 100；多池取最高分与共振逻辑含动量池。配置与实现见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md) §3.3。

#### 可选过滤器与调制

- **流动性**（`filters.liquidity.enabled`）：20 日日均成交额分位得到 liquidity_score；可作过滤或对 technical_score 调制。
- **波动率 Regime**（`filters.volatility_regime.enabled`）：ATR(14)/close 分位 > 阈值时对得分乘以 penalty_ratio。
- **板块强度**（`filters.sector_strength.enabled`）：同板块 technical_score 均值/全市场均值，可参与过滤或调制。

扫描输出除 technical_score、strategy_source、sector_strength 外，包含 **second_pool_id**、**second_pool_score**、**pool_scores**（各池得分）、**liquidity_score**、**volatility_ratio**，供下游判官/Kelly 等按策略维度使用。见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md) §4。

### Sector Strength（板块强弱分析）

**功能**：计算个股相对板块的强弱，剔除跟风杂毛。板块划分与 Module A 行业数据**同源**（如申万）；生产级数据要求与验收见 [11_ 生产级数据要求](./11_数据采集与输入层规约.md)、[Stage2 06_生产级数据要求](../Stage2_数据采集与存储/06_生产级数据要求_设计.md)。
...
class QuantScanner:
    ...
    def scan_market(self, universe: List[str]) -> List[QuantSignal]:
        """
        扫描全市场
        :param universe: 当前全 A 股标的池（由 get_current_a_share_universe() 获取或调用方传入）
...
**说明**：三池得分为**每条件 0–50 连续子分、两条件相加 cap 100**；取三池最高分为 technical_score，若至少两池≥40 则加多池共振 bonus（见 [02_B模块策略_策略实现规约](../Stage3_模块实践/02_B模块策略_策略实现规约.md)）。technical_score 为 [0,100] 实数。
...
**实现模式**：**按股配置 + 统一分析**。每只标的按领域与主营细分配置数据（segment_list、segment_signals），经**统一分析管道**（对齐、聚合、认知边界与结构化维度）输出**一条**专家意见；不按领域分多套专家实现。入口为 `unified_opinion`；标签「未知」或未在 supported_tags 时输出一条「不支持」。

### 输入/输出（含右脑数据支撑扩展）

| 输入 | 输出 |
|------|------|
| Module A 的 Tag、Module B 的 QuantSignal | 专家意见（ExpertOpinion）列表，供 D 投票与 Kelly 计算 |
| **扩展**（见 [12_右脑数据支撑与细分规约](./12_右脑数据支撑与Segment规约.md)）：(a) 标的的细分列表（来自 A 的主营占比列表或 L2 标的主营构成表）；(b) 各细分的垂直一手信号（L2 细分信号缓存表，由信号层按候选按需拉取） | — |

**专家逻辑**：须实现「利好与主营对齐」判断（利好是否落在该标的主营细分上）、多细分聚合规则（主营细分一票否决或按营收占比加权）；详见 12_、dna_module_c。

**输出语义（自然语言）**：每条专家意见包含——**标的代码**、**是否支持**（在能力范围内且可看多为真，否则为假）、**信号方向**（看多/看空/中性）、**确信度**（0～1）、**理由摘要**（人话说明）、**风险提示**（列表）、**期限类型**（短线或长线）。判官用「是否支持」「方向」「确信度」做投票与凯利计算。

**C 模块设计要求**：理由摘要须含可解析的**结构化维度**（对齐得分、景气强度、风险等级、利好强度）；按风险等级对确信度降权（高×0.5、中×0.9、低不降权）。详见 [04_A轨_MoE议会_设计#C 模块设计要求](../../03_原子目标与规约/Stage3_模块实践/04_A轨_MoE议会_设计.md#design-stage3-04-requirements)。

### 入口与 Router（按股配置 + 统一分析）

- **统一分析入口**：`unified_opinion(symbol, quant_signal, segment_list, segment_signals, config, domain_tag)`，按设计文档管道步骤输出一条 ExpertOpinion；domain_tag 仅用于选用 risk_factor_templates 与 domain 枚举。
- **Router**：config 中 `supported_tags`（如 农业、科技、宏观）；domain_tags 中首个在 supported_tags 的 tag 调用 unified_opinion，否则返回一条 `trash_bin_opinion`（is_supported=False）。每标的一条意见。**A 轨**输出 horizon=SHORT_TERM；**B 轨（中线）**候选走同一管道、输出 horizon=MEDIUM_TERM（见 [04_B轨_C模块右脑与信号设计](../Stage3_模块实践/04_B轨_C模块右脑与信号设计.md)）；**C 轨（长线）**可叠加 VC-Agent 意见、输出 LONG_TERM。

### 统一分析管道（无分专家）

逻辑唯一：对齐、多细分聚合、认知边界、利好强度/景气强度/风险分级/结构化摘要均在同一管道内完成。**domain_tag**（农业/科技/宏观）仅用于从 config 的 `risk_factor_templates` 选用风险提示文案，不改变计算。详见 [04_A轨_MoE议会_设计#策略详规与实现细粒度](../../03_原子目标与规约/Stage3_模块实践/04_A轨_MoE议会_设计.md#design-stage3-04-strategy)。

#### VC-Agent（信仰专家 / 长期捕手轨）

**定位**：专家池可包含**信仰专家（VC-Agent）**，服务于三轨制中的**长期捕手轨（C 轨）**。VC-Agent 输出的 `ExpertOpinion` 应设置 `horizon = TimeHorizon.LONG_TERM`，以便判官（Module D）对该轨施加差异化规则（豁免 2% 硬止损与现金拖累）。

**数据与逻辑**：Repo-I 中 VC-Agent 可接入**基本面数据**（如 AkShare 财报、营收增速、研发投入占比），与 A 轨技术面信号区分；逻辑为逻辑证伪止损、买入锁仓至大周期反转，详见 L1 [03_双轨制与VC-Agent](../../01_顶层概念/03_双轨制与VC-Agent.md)。Router 可根据标的或资金账户类型将请求路由至 VC-Agent，与 Agri/Tech/Geo 等短轨专家并行或按配置选用。

**Router 与 VC-Agent 衔接（规约）**：
- **路由条件**：根据标的类型（如是否纳入 C 轨标的池）、资金账户/资金池标识（A/B/C 轨）、以及配置开关（如 `enable_vc_agent`）决定是否将请求路由至 VC-Agent。
- **与 Agri/Tech/Geo、B 轨的关系**：B 轨（中线）候选走右脑统一分析、输出 MEDIUM_TERM；C 轨（长线）候选走 VC-Agent、输出 LONG_TERM；互斥或并行由配置决定。
- **判官消费**：Module D 读取 `ExpertOpinion.horizon`；若为 `MEDIUM_TERM` 则走 B 轨分支（中线止损、阶段止盈）；若为 `LONG_TERM` 则走 C 轨分支（豁免 2% 与现金拖累，仅施加逻辑证伪与大周期反转），见下 Module D 三轨分流。
- **C 轨专用**：输入契约、共识 schema、C 轨候选表与 L4 实践见 [03_/C轨/](../C轨/README.md)。
...
**规定**：判官根据 `TimeHorizon`（协议见 expert.proto，需支持 SHORT_TERM / MEDIUM_TERM / LONG_TERM）分支执行：

- **SHORT_TERM（A 轨）**：现金奶牛轨；施加 2% 硬止损、现金拖累、不可能三角约束。
- **MEDIUM_TERM（B 轨）**：中线捕手轨；**不**施加 2% 硬止损与现金拖累；施加**中线止损**（如 5%～8% 或 ATR/结构，可配置）与**阶段止盈**（如 30%/50%/100% 或阶段结束/景气拐点）。详见 [03_/B轨/00_B轨整体设计](../B轨/00_B轨整体设计.md)、[01_B轨系统设计](../B轨/01_B轨系统设计.md)。
- **LONG_TERM（C 轨）**：长期捕手轨；豁免 2% 硬止损与现金拖累；仅施加逻辑证伪止损与大周期反转止盈。具体条件见 L1 [03_三轨制与TimeHorizon](../../01_顶层概念/03_双轨制与VC-Agent.md)。**C 轨专用**输入/输出与接口见 [03_/C轨/03_C轨共识与逻辑证伪规约](../C轨/03_C轨共识与逻辑证伪规约.md)。
...
<a id="module-e-minimal-data-interfaces"></a>

### 最小数据接口（只读持仓、只读行情）

对 **A 轨已持仓** 的复核、止损合并与盘中硬止损，Module E 依赖两类**只读**输入，**不由 Module B 提供持仓或账户真相**：

| 接口 | 用途 | 说明 |
|------|------|------|
| **只读持仓 API** | 当前未平仓列表及字段（symbol、方向、数量、入场价、当前有效止损等） | 真相源为 Broker / OMS / 持仓表；字段最小集见 [03_A轨_持仓与每日信号复核规约](../Stage3_模块实践/03_A轨_持仓与每日信号复核规约.md#design-a-track-pl-minimal-data) |
| **只读 last price** | 现价相对入场/止损，用于盘中触发 | 可为 last / mid / 标记价之一，产品固定一种；**日频**复核可主要依赖每日 B + 持仓快照，**盘中**硬止损则**必须**有行情 |

抽象落点见 [05_接口抽象层规约](./05_接口抽象层规约.md)（BrokerDriver、MarketDataFeed）；DNA 键见 `core_modules/dna_a_track_position_lifecycle.yaml` 的 `minimal_data_interfaces`。

### A 轨实现层风控约束（与 L1 生存底线对应）

L1 生存底线「回撤 < 12%」的实现层风控数值由本规约与 [05_成功标识与验证/02_验收标准](../../05_成功标识与验证/02_验收标准.md) 约定，**仅适用于 A 轨**（B 轨、C 轨豁免见 Module D 三轨分流）：

| 约束项 | 权威数值 | 实施位置 | 验收 |
|--------|----------|----------|------|
| **单笔风险上限** | 单笔亏损 > 2% → 市价斩仓 | Module E 硬止损 | L5 不可能三角验收「单笔风险 < 2%」、`l5-func-03` |
| **盈亏比下限** | 预期收益/风险 < 1.5 → 拒绝开仓 | Module E 开仓前检查 | L5 不可能三角验收「盈亏比 > 1.5」、`l5-func-03` |

实现须与下文「风控规则」一致；调整数值须经 ADR 或 L3 规约变更，并同步 L1/L2 引用。
...
logger.info(
    "scan_completed",
    # universe 以 get_current_a_share_universe() 返回为准，不写死数量
...
class TestLoadPerformance:
    ...
    def test_scanner_load(self):
        """扫描引擎压力测试"""
        # 当前全 A 股（universe）扫描，要求 < 30 分钟
        start_time = time.time()
        universe = get_current_a_share_universe()  # 或测试用 mock 列表
        results = scanner.scan_market(universe)
...
<a id="回测仿真引擎职责与仿真度统一入口"></a>
## 回测/仿真引擎职责与仿真度（统一入口）

回测与仿真路径的**职责边界**与**仿真度要求**以 [Lean (QuantConnect)](https://github.com/QuantConnect/Lean) 四层解耦为架构参照，与 [01_顶层概念/05_谛听优先借鉴的十大开源选型](../../01_顶层概念/05_谛听优先借鉴的十大开源选型.md) 一致。具体四层与 Module A～F 的映射、仿真度选项（时间粒度、滑点、手续费、公司行动）、当前实现级别与可升级项，见 [Stage4_MoE与执行网关/03_回测或仿真验证设计](../Stage4_MoE与执行网关/03_A轨_回测或仿真验证_设计.md#design-stage4-03-integration-lean)。本规约仅作统一入口引用，详细设计与验收以 Stage4-03 为准。
