...
- **Router**：config 中 `supported_tags`（如 农业、科技、宏观）；domain_tags 中首个在 supported_tags 的 tag 调用 unified_opinion，否则返回一条 `trash_bin_opinion`（is_supported=False）。每标的一条意见（短轨）；B 轨可叠加 VC-Agent 意见。
...
**定位**：专家池可包含**信仰专家（VC-Agent）**，服务于双轨制中的**长期捕手轨（B 轨）**。VC-Agent 输出的 `ExpertOpinion` 应设置 `horizon = TimeHorizon.LONG_TERM`，以便判官（Module D）对该轨施加差异化规则（豁免 2% 硬止损与现金拖累）。
...
- **路由条件**：根据标的类型（如是否纳入 B 轨标的池）、资金账户/资金池标识（A 轨或 B 轨）、以及配置开关（如 `enable_vc_agent`）决定是否将请求路由至 VC-Agent。
- **与 Agri/Tech/Geo 的关系**：互斥或并行由配置决定；典型为「标的属于 B 轨池则仅走 VC-Agent，否则走短轨专家」或「先短轨筛选再可选 VC-Agent 二次意见」。
- **判官消费**：Module D 读取 `ExpertOpinion.horizon`；若为 `LONG_TERM` 则走 B 轨分支（豁免 2% 与现金拖累，仅施加逻辑证伪与大周期反转），见下 Module D 双轨分流。
- **B 轨专用**：输入契约、共识 schema、B 轨候选表与 L4 实践见 [03_/B轨/01_B轨系统设计](../B轨/01_B轨系统设计.md)、[03_B轨共识与逻辑证伪规约](../B轨/03_B轨共识与逻辑证伪规约.md)。
...
**规定**：当输入信号或专家意见携带 `TimeHorizon = LONG_TERM`（协议见 expert.proto）时，判官对**长期捕手轨（B 轨）**施加差异化规则：

- **豁免**：不施加 2% 硬止损规则（单笔亏损 > 2% 不触发市价斩仓）；不施加现金拖累监控（Cash Drag Monitor 不作用于该轨仓位）。
- **仍施加**：逻辑证伪止损（产业趋势或基本面逻辑被证伪时止损）、大周期反转止盈（Major Trend Reversal 时止盈）。具体条件见 L1 [03_双轨制与VC-Agent](../../01_顶层概念/03_双轨制与VC-Agent.md) 或下游规约。**B 轨专用**输入/输出与接口见 [03_B轨共识与逻辑证伪规约](../B轨/03_B轨共识与逻辑证伪规约.md)。

当 `TimeHorizon` 为 `SHORT_TERM` 或未设置时，按现有规则执行（现金奶牛轨 A 轨），受不可能三角与 2% 止损、现金拖累约束。
...
L1 生存底线「回撤 < 12%」的实现层风控数值由本规约与 [05_成功标识与验证/02_验收标准](../../05_成功标识与验证/02_验收标准.md) 约定，**仅适用于 A 轨**（B 轨豁免见 Module D 双轨分流）：
