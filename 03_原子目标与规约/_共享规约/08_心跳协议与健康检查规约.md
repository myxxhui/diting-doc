...
> - **战略维度**: [可观测性与运行保障](../../02_战略维度/07_平台与产品/04_可观测性与运行保障.md)
> - **原子规约**: [全链路通信协议矩阵](./04_全链路通信协议矩阵.md)
> - **对应 DNA**: `global_const.yaml#heartbeat_protocol`
...
实盘多服务部署时，须支持**按请求串联**各服务日志与指标，便于排查与审计。

| 要求 | 约定 |
|------|------|
| **追踪 ID** | 全链路使用 `correlation_id`（或与 `trace_id` 统一）；从入口生成，各服务透传并在日志与 span 中输出。与 [04_ 全链路通信协议矩阵](./04_全链路通信协议矩阵.md) 跨服务字段一致。 |
| **日志格式** | 结构化日志（如 JSON）；每条日志须含 `correlation_id`、`service_name`、`timestamp`、级别与业务字段。 |
| **串联方式** | 可通过 OpenTelemetry/W3C TraceContext 或应用层透传 correlation_id；多服务一次请求可在同一 trace 下查询。 |
...
```python
# diting-src/diting/heartbeat/monitor.py
...
```python
# diting-src/diting/heartbeat/sender.py
import asyncio
...
```python
# diting-src/diting/heartbeat/execution_sender.py
...
```python
# diting-src/diting/heartbeat/circuit_breaker.py
...
```python
# diting-src/diting/heartbeat/execution_breaker.py
...
```python
# diting-src/diting/heartbeat/health_check.py
...
```python
# diting-src/diting/heartbeat/metrics.py
