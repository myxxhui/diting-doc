...
| 模块 | Proto 文件 | 说明 |
|------|------------|------|
| **Module A** | `classifier/classifier_output.proto` | 语义分类器输出：Domain Tag、置信度；**扩展** 主营占比列表 segment_shares（见下「ClassifierOutput 扩展」）；见 [09_核心模块架构规约](./09_核心模块架构规约.md)、[12_右脑数据支撑与细分规约](./12_右脑数据支撑与Segment规约.md) |
...
### ClassifierOutput 扩展（右脑数据支撑）

为支持 Module C 右脑「利好与主营对齐」判断，ClassifierOutput 须增加**可选**字段**主营占比列表**（segment_shares），与 L2 标的主营构成表一致；详见 [12_右脑数据支撑与细分规约](./12_右脑数据支撑与Segment规约.md)。

在 `classifier_output.proto` 中增加（若尚未存在）；字段含义：细分标识、营收占比、是否主营。
...
- 与 L2 标的主营构成表一行对应一个 SegmentShare；同一标的多行即 repeated。
...
enum ExpertDomain {
  ...
  DOMAIN_AGRI = 1;      // 农业（猪周期、气候）；存储与 DB 用「农业」
  DOMAIN_TECH = 2;      // 硬科技（半导体、AI）；存储与 DB 用「科技」
  DOMAIN_MACRO = 3;     // 宏观（汇率、大宗）；存储与 DB 用「宏观」
...
| 报文字段 | 含义 | 约束 |
|----------|------|------|
| 标的代码 | 哪只股票 | 与左脑量化信号标的对齐 |
| `is_supported`（是否支持） | 在能力范围内且结论可看多为真；不懂或不该上为假 | 为假时判官直接否决 |
| `direction`（信号方向） | 看多 / 看空 / 中性 | 至少一条支持且看多才与左脑通过投票 |
| `confidence`（确信度） | 0～1，越大越有信心 | 判官用于动态凯利算仓位 |
| `reasoning_summary`（理由摘要） | 人话说明 + **须含**「对齐得分=xx 景气强度=xx 风险等级=xx 利好强度=xx」 | 必须非空，用于审计与解析 |
| `risk_factors`（风险提示） | 可能的风险点列表 | 用于风控与展示 |
| `horizon`（期限类型） | 短线（A 轨）或长线（B 轨） | LONG_TERM 时判官走 B 轨（豁免 2% 与现金拖累） |
