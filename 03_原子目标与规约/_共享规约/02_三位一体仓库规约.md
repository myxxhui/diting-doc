...
**实践测试方式约定**：以上流程为**主要（默认）**实践测试方式；各 Stage 下 04_ 实践文档须以本地 Docker Compose（或等价本地环境）为**默认验收路径**，K3s/实盘为可选路径；无云凭证或无需真实集群时优先使用本地 Compose 完成准出。见 [00_系统规则_通用项目协议](../../../00_系统规则_通用项目协议.md)#实践测试方式约定l4-全局。
