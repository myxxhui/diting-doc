...
**原则**：**diting-infra** 负责所有与「环境、编排、建表、部署」相关的制品；**diting-core** 仅负责业务代码与面向已就绪环境的连接/验证。
...
| **docker-compose.module-a.yaml**（Module A 本地验证） | **diting-infra** | 用于 Stage3-01：在部署到 K3s 前本地用 Compose 运行 Module A 镜像、连接远程库验证；见 diting-infra `compose/docker-compose.module-a.yaml`；镜像在 diting-core 构建（make build-module-a） |
...
| **local-deps-up / local-deps-down / local-deps-init** | **diting-infra** | 启动/回收 Compose、执行本地建表；开发者在 **diting-infra** 执行后再在 **diting-core** 配置 .env 并运行 verify-db-connection、ingest-test |
| **verify-db-connection、ingest-test** | **diting-core** | 使用 .env 连接 L1/L2（localhost 时由 infra 已启动的 Compose 提供）；不包含编排与建表 |

**实践流程**：① 在 **diting-infra** 执行 `make local-deps-up`、`make local-deps-init`；② 在 **diting-core** 配置 `.env` 指向 localhost:15432/15433（或 infra 约定端口）；③ 在 **diting-core** 执行 `make verify-db-connection`、`make ingest-test`。回收时在 **diting-infra** 执行 `make local-deps-down`。
