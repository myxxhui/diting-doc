...
| 细分标识 | 领域 | 子领域 | 中文名 | 信号适配器标识 |
|----------|------|--------|--------|----------------|
| agri_pork_cycle | 农业 | 生猪 | 猪周期 | 可选 |
| agri_grain | 农业 | 粮食 | 粮食种植 | 可选 |
| tech_semiconductor_fab | 科技 | 半导体制造 | 半导体制造 | 可选 |
| tech_semiconductor_design | 科技 | 半导体设计 | 半导体设计 | 可选 |
| tech_ai_compute | 科技 | AI算力 | AI算力 | 可选 |
| geo_copper | 宏观 | 铜 | 铜/有色金属 | 可选 |
| geo_crude | 宏观 | 原油 | 原油 | 可选 |
...
- **逻辑**：
  ...
  3. 对每个标的写入**标的主营构成表**：每行（标的、细分标识、营收占比、是否主营、来源、更新时间）；同一标的下营收占比最高的一行设为主营。
...
- **实现归属**：可在 diting-core 内独立服务或与编排/Module C 同进程；接口约定见下。
