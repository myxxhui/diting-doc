...
- **segment_tier**：`1` = domain（领域层）、`2` = sector（板块/赛道）、`3` = business（具体业务）。主营披露哈希 `seg_bp_*` 行在采集写入时 **默认 3**（与 `diting.ingestion.segment_tier` 一致）。
- **sub_domain**：申万行业名或归一化赛道名，与 **domain（三分类）正交**；报表与 `industry_revenue_summary` 并列展示。
...
| 采集入口 | `diting.ingestion.business_profile.run_ingest_business_profile`；数据源 **AkShare `stock_zygc_em`**，取**最新报告期**且 **`分类类型=按产品分类`** 的分部行；`segment_id=seg_bp_{md16}` 稳定键；`segment_registry.domain` 由同标的 `industry_revenue_summary.industry_name` 粗映射（农业/科技/宏观）；**sub_domain** 写入同申万行业名（与 domain 正交）；**segment_tier=3**（披露业务层）；见 `diting.ingestion.segment_tier` |
