...
flowchart TB
    subgraph 配置源[配置源：Repo-G]
        YAML[diting-src/design/rules/（或 diting-doc 下 _Design_Artifacts/rules/，见 02_三位一体仓库规约）<br/>strategy_params.yaml]
...
| **L2: 业务规则** | 策略参数（MA周期、止损比例） | `diting-src/design/rules/（或 diting-doc 下 _Design_Artifacts/rules/，见 02_三位一体仓库规约）` | 中频 | **热更新** |
...
```
diting-src/design/rules/（或 diting-doc 下 _Design_Artifacts/rules/，见 02_三位一体仓库规约）
...
```yaml
# diting-src/design/rules/strategy_params.yaml（或 _Design_Artifacts/rules/，见 02_三位一体仓库规约）
strategies:
  Trend_MA:
...
```python
# diting-src/diting/config/watcher.py
...
```python
# diting-src/diting/strategy/trend_ma.py
...
```python
# diting-src/diting/config/manager.py
class ConfigManager:
...
```python
# diting-src/diting/config/validator.py
