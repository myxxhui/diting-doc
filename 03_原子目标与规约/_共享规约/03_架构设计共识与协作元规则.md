...
> - **对应 DNA**: `global_const.yaml#architecture_consensus`
...
**维度归属说明**：本规约中 **B. 金融级 GitFlow / 三级流水线** 属于 **开发与交付** 维度，与 [01_开发生命周期与实践流程规约](../节奏与交付/01_开发生命周期与实践流程规约.md) 衔接；**A. ADR 强制令** 与 **5D、SemVer** 等为产品与交付 **共用** 的协作与设计约束。
...
**规则**：在修改 `diting-src` 的任何核心逻辑（如：更换量化指标、调整止损算法、接入新券商）之前，必须在 diting-doc 的 `06_追溯与审计/ADR/` 提交一个 ADR（架构决策记录，Architecture Decision Record）。
...
> **与开发生命周期的关系**：开发生命周期（见 [01_开发生命周期与实践流程规约](../节奏与交付/01_开发生命周期与实践流程规约.md)）定义从零到可提交的实践顺序（骨架期 → 逻辑填充期 → Docker 期 → K3s 测试开发期）；本小节定义**提交后**的流水线（commit → Level1 单测；merge develop → Level2 Staging；tag → Level3 Live）。
...
**Level 2 回滚触发**：净值曲线与回测偏差 ≥ 1%、或关键告警（如风控盾失效、心跳超时）触发时，须自动或人工触发回滚至上一可用版本；回滚后须重新跑 Level 2 验证并通过后再考虑重新进入 Level 3。
...
**Level 3 回滚触发**：生产出现 P0 故障（如通道不合规、密钥泄露风险、连续 RTO 超标）、或业务要求紧急回退时，由审批权责人决定回滚；回滚须使用上一 Tag 的 release bundle，回滚后须验证健康检查与关键业务指标，并记录回滚原因与时间。

**准出后移交运维**：Level 3 发布准出后，交付方须完成移交运维检查项，责任归属为发布负责人或项目负责人。检查项包括：Runbook 与故障分级/事件响应表可访问、监控与告警接收人已配置、密钥与配置清单已交接（按最小知情原则）、[10_运营治理与灾备规约](./10_运营治理与灾备规约.md) 生产就绪检查与事件响应已就绪。L5 验收见 [02_验收标准](../../05_成功标识与验证/02_验收标准.md)「移交运维准出」行。
...
| 级别 | 规约目标触发 | 当前实施方式 |
|------|--------------|--------------|
| **Level 1** | git commit 触发单元测试 | 由开发者**本地执行 `make test`**（diting-src）代替；无自动触发。 |
