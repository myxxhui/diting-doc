...
> - **战略维度**: [数据体系与分层存储](../../02_战略维度/07_平台与产品/03_数据体系与分层存储.md)
...
message DecisionOutput {
  Verdict verdict = 1;                 // 决策裁决（历史 proto 注释或作 Verdict）
...
```python
# diting-src/diting/data/versioned_query.py
...
```python
# diting-src/diting/data/metadata.py
...
```python
# diting-src/diting/data/versioned_writer.py
...
```python
# diting-src/diting/data/integrity_checker.py
