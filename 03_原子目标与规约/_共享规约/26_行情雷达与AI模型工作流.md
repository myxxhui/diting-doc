...
flowchart TB
  ...
  T2 -.->|HTTPS_PROXY| PX --> AN[(Anthropic API)]
  Chat -.->|HTTPS_PROXY| PX --> AN
```
...
2. **浏览器「启动扫描」**（标的 6 位代码，如 601138）  
   ...
   - **T2**（勾选深度推理）：经 **HTTPS_PROXY** → 新加坡 → **Opus** 输出 9 维 JSON  
...
| `HTTPS_PROXY` | `http://ditingproxy:***@<新加坡IP>:3128` | 香港 pod 访问 Opus 必填 |
| `NO_PROXY` | 含 `.svc`、内网 CIDR | 集群内 Redis/PG 不走代理 |
