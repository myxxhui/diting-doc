...
<a id="design-32-cognition-engine"></a>

### §1.5 生意认知引擎（Z1.5 · Z1→Z2 深度认知过渡层）

> **定位**：Z1 九维 Gate-A 通过后、Z2 买入论证台前，对标的执行深度认知分析——将九维分数转化为对生意的深度理解。这是段永平"看懂生意"能力在系统内的工程化落点。

#### §1.5.1 为什么需要 Z1.5

Z1 九维 Gate-A 做的是**广度粗筛**（打分制）——它问的是"这个标的几分"。但段永平的投资逻辑基础是"买股票就是买公司"——九维分数高 ≠ 你看懂了这家公司。

| Z1 九维能做 | Z1.5 认知引擎能做 |
|------------|-----------------|
| D1（行业地位）2 分 | 这个地位是怎么来的？能持续吗？ |
| D2（技术壁垒）2 分 | 用户在什么情况下会叛逃到竞品？ |
| D3（成长空间）2 分 | 这个增长来自用户真实需求还是政策驱动？ |
| D7（治理结构）0 分 | 管理层过去 5 年言行是否一致？ |
| D9（预期差）2 分 | 我有哪些不确定性是还没意识到的不确定性？ |

#### §1.5.2 产品工作区命名与 Z1.5 工程区码

| 工程区码 | 产品工作区名 | 用户价值一句话 |
|---------|------------|-------------|
| **Z1.5** | **生意认知坊** | 把九维分数变成对生意的深度理解，写一份"我真的懂了吗"的自检报告 |

**代码标识**：`workspace_id = "cognition_lab"`（英文 slug · 稳定）  
**提醒类目**：`cognition_complete | cognition_conditional_pass | negation_veto | cross_step_conflict`  
**前端入口**：`view=planning` 下「认知分析」子 Tab（消费 cognition_pack + Gate-A.5 结果）

#### §1.5.3 认知引擎与段永平 7 条核心哲学的映射

| 段永平原则 | 原话 | 认知引擎对应步 |
|-----------|------|-------------|
| 买股票=买公司 | ⑤ | **step1 用户价值扫描**——这个生意到底解决了用户的什么真问题 |
| 诚信内生 | ②⑦ | **step2 管理团队画像**——管理层 3 年言行一致性追踪 |
| 护城河不可替代 | ⑪ | **step3 护城河五力分析**——护城河=不可替代性+长期一致性 |
| 不懂不做 | ⑩ | **step6 认知边界检查**——明确列出还不知道的 3 件事 |
| 价值回归 | ④ | **step5 估值安全边际（消费 step_11）**——三档情景推演 |
| 平常心=理性 | ③ | **step9 未知→已知**——把无知转化为追踪任务 |
| 吃亏只一次 | ⑦ | **step0 否定清单**——诚信/质量/认知三重一票否决 |
...
flowchart TB
  ...
  Z1_5["生意认知坊 Z1.5<br/>九步深度认知 · Gate-A.5"]
  Z2["买入论证台 Z2<br/>证据包 · 论文 · Gate-B"]
  Z3["持仓监护室 Z3<br/>监护 · Gate-C/D"]
  Z4["决策复盘库 Z4<br/>影子账本 · 探针体检"]
  Z0 -->|"扫描清单 + P0"| Z1
  Z1 -->|"Gate-A 通过 · 推荐池"| Z1_5
  Z1_5 -->|"Gate-A.5 通过 · BusinessCognitionPack"| Z2
  Z2 -->|"论文 + 监控网"| Z3
  Z3 -->|"决策日志"| Z4
  Z4 -.->|"校准回流"| Z0
  Z4 -.-> Z1
...
**漏斗淘汰率基准**：Z1 收 100 → Z1.5 入 15～20 → Z2 留 8～12 → Z3 持 1～3（单人带宽现实上限）。
...
| **Z1.5** | **生意认知坊** | 深度认知——把九维分数转化为对生意的深度理解 | 生意分析师（段永平思维模拟 · 9 步流程） | **`BusinessCognitionPack`**：含否定清单 + 六步分析 + 跨步关联 + 置信度汇总 + 未知→已知 | **Gate-A.5**：否定清单全过 + 六步全绿 + 跨步关联 ≥3 条 + 认知边界非 fail（见 §2.4.8） |
...
<a id="design-32-z1_5-cognition-workflow"></a>

#### §2.4.8 Z1.5 · 生意认知引擎工作流（9 步深度认知）

> **定位**：Z1 Gate-A 通过后、Z2 模板 A 起草前，对每个 Gate-A 通过的标的执行完整的生意深度认知分析。

**流程总览**：

```mermaid
flowchart LR
  Z1_Z1_5["Z1 Gate-A 通过<br/>15～20 标的"] --> S0["Step0 否定清单<br/>前置一票否决"]
  S0 -->|pass| S1["Step1 用户价值"]
  S1 --> S2["Step2 管理团队"]
  S2 --> S3["Step3 护城河"]
  S3 --> S4["Step4 现金流推演"]
  S4 --> S5["Step5 估值安全边际<br/>（消费 step_11）"]
  S5 --> S6["Step6 认知边界"]
  S1 & S2 & S3 & S4 & S5 & S6 --> S7["Step7 跨步关联<br/>（模拟段永平全局直觉）"]
  S7 --> S8["Step8 置信度汇总<br/>三级标记 + 降权"]
  S8 --> S9["Step9 未知→已知<br/>信息收集任务生成"]
  S9 --> GA5["Gate-A.5"]
  GA5 -->|pass| Z2["Z2 买入论证台"]
  GA5 -->|conditional_pass| Z2_C["Z2（标注认知边界）"]
  GA5 -->|fail| DROP["丢弃 / 退回 Z1 观察"]
  S0 -->|veto| DROP
```
...
| 消费对象 | 来源 | 用途 |
|---------|------|------|
| `nine_dim_scorecard` | Z1 Gate-A 产出 | 九维分数，认知引擎读取但不重新打分 |
| `evidence_pack` 摘要 | Z2 evidence_pack_builder（已有） | 财务数据，供 step2/step4 引用 |
| `ValuationDynamics` | step_11 估值动态评估器 | 戴维斯 4 档 + PE 分位 + 行业对比，供 step5 消费 |
| `cvm_scorecard_slice` | Z0 CVM 产出 | 生态位锚点信息，供 step3 语境 |
| `scan_dispatch` | Z0 产出 | theme/phase_id/ecosystem_e1_e5，供 step1 语境 |

**产出**（Z1.5 执行后交付 Z2）：

| 产出对象 | Schema | 消费方 |
|---------|--------|--------|
| `business_cognition_pack.json` | BusinessCognitionPack（见 §10.8） | Z2 模板 A · Gate-A.5 · Z3 监控 |
...
| 类别 | 否决项 | 触发条件 |
|------|--------|---------|
| **诚信类** | 财务造假记录 | 管理层在上市公司/关联公司有过造假 |
| **诚信类** | 减持信号 | 大股东频繁减持且理由模糊 |
| **诚信类** | 关联交易掏空 | 关联交易金额 > 30% 营收且定价不公 |
| **诚信类** | 产品安全事故 | 重大安全事故且处理方式遮掩 |
| **业务类** | 靠政策驱动 | 增长超过 50% 来自政府补贴/政策红利 |
| **业务类** | 持续烧钱 | 需要持续大规模资本投入维持现有地位 |
| **业务类** | 不懂核心 | 无法用一句话说清楚公司怎么赚钱 |
| **业务类** | 现金流出问题 | 经营性现金流连续 3 年为负 |
| **业务类** | 没有护城河 | 行业进入壁垒低到"谁都能做" |
| **认知类** | 跟风买入 | 买它的唯一理由是"别人都在买"/"价格在涨" |
| **认知类** | 不了解行业 | 看了这个行业不到 1 个月 |
| **认知类** | 身家赌注测试 | 回答"我愿意把全部身家投进去吗"为 no |

> **执行规则**：Step0 必须先于 Step1 执行；否决触发 → 直接丢弃，不入 Z2；否决日志写入 `cognition_pack` 供 Z4 审计。

**Step1 用户价值扫描**：

- **AI 角色**：产品分析师（模拟段永平"质量=一致性，由用户需求定义"）
- **核心问题**：这个生意到底解决了用户的什么"真问题"？
- **分析任务**：① 列举 3 个用户核心场景和痛点；② 这款产品/服务如何解决；③ 如果没有它，用户怎么办（衡量不可替代性）；④ 用户愿意付多少钱（支付意愿上限）
- **输出**：`UserValueAnalysis`（含 real_problem, user_scenarios, irreplaceability, payment_willingness）
- **段永平原话映射**：① "质量=一致性，标准由用户需求定义"

**Step2 管理团队画像**：

- **AI 角色**：管理层审计员（模拟段永平"诚信发自内心，吃亏只吃一次"）
- **核心问题**：掌管这个生意的人值不值得信任？
- **分析任务**：① 过去 5 年说了什么、做了什么、兑现了什么；② 是否长期增持/持有股份；③ 有无重大诚信争议；④ 危机处理时的决策记录
- **输出**：`ManagementPortrait`（含 integrity_score, consistency_score, crisis_handling, capital_allocation, risk_signals）
- **段永平原话映射**：②⑦ "诚信发自内心" + "吃亏永远只吃一次"

**Step3 护城河五力分析**：

- **AI 角色**：竞争策略分析师（模拟段永平"不可替代性+长期一致性"）
- **核心问题**：别人能不能轻易复制？用户在什么情况下会叛逃？
- **分析任务**：① 列出 3-5 个核心竞争优势；② 评估复制成本/时间；③ 用户切换成本量化；④ 过去 3 年新进入者挑战情况
- **输出**：`MoatAnalysis`（含 advantages[], switching_cost, defection_scenarios, depth_score）
- **段永平原话映射**：⑪ "定价权就是谁求谁；护城河=不可替代性+长期一致性"

**Step4 现金流质量推演**：

- **AI 角色**：财务分析师（模拟段永平"看懂未来的现金流"）
- **核心问题**：这个生意赚的钱是不是真的、会不会持续、会不会增长？
- **分析任务**：① 经营现金流 vs 净利润 5 年差距；② 核心驱动因子识别；③ 驱动因子未来 3-5 年变化推演；④ 资本再投入强度
- **输出**：`CashflowProjection`（含 historical_quality, core_drivers, projections[悲观/中性/乐观], capex_intensity）
- **段永平原话映射**：⑤ "看懂未来的现金流"

**Step5 估值安全边际（消费 step_11）**：

- **AI 角色**：估值仲裁员——**禁止自行计算估值数字**，只做综合判断
- **核心问题**：现在的价格，买了会不会睡不着？
- **输入来源**：**仅来自 step_11 `ValuationDynamics`**（davis_phase + pe_percentile_* + industry_median_pe + peg）
- **AI 职责**：基于 step_11 的纯数学结论，结合 step1-4 的认知结论，综合判断"这个价格是否提供了足够安全边际"
- **输出**：`ValuationMargin`（含 current_pe_percentile_3y, floor_price[悲观情景], margin_of_safety_pct）
- **段永平原话映射**：④ "十块掉到一块不是亏，是因为你知道它值十块"
- ⚠️ **防偏离**：step5 的 prompt 必须显式声明 `DATA_SOURCE: step_11`，禁止自行计算 PE/分位/Davis

**Step6 认知边界检查**：

- **AI 角色**：自我审计员（模拟段永平"追问为什么，不懂就不做"）
- **核心问题**：我有什么还没搞懂的？
- **分析任务**：① 列出 3 件不确定的事；② 每件需要什么信息、从哪里获取；③ 如不消除能否接受；④ 有没有看不懂的核心部分
- **输出**：`CognitiveBoundary`（含 uncertainties[], incomprehensible_parts[], verdict: pass|conditional_pass|fail）
- **段永平原话映射**：⑥⑩ "追问为什么" + "不懂就不做"

**Step7 跨步关联 Agent（模拟段永平全局直觉）**：

- **AI 角色**：系统性思维审计员
- **核心问题**：六步分析之间是否存在矛盾？是否存在"每步都对、整体错"的情况？
- **强制关联模板**（必须输出 ≥3 条）：
  1. **step1↔step3**：用户价值 vs 护城河——护城河是"用户真正依赖的高切换成本"还是只是"供应链壁垒"？
  2. **step2↔step4**：管理层诚信 vs 现金流质量——诚信分高但现金流差，是能力问题还是阶段问题？
  3. **step1↔step6**：你说懂 vs 你承认不懂——如果 step6 标注了不懂，你怎么确定 step1 识别的"真问题"就是真正的真问题？
- **输出**：`cross_step_correlations[]`，每条含 `{step_a, step_b, correlation_finding, conflict_found: bool, investment_impact}`
- ⚠️ **防偏离**：跨步关联不是"总结"，是"发现矛盾"。如果输出为 0 条矛盾 → 标记 `CROSS_STEP_SUSPICIOUS`
...
- **三级标记**：
  | 标记 | 条件 | Gate-A.5 降权 |
  |------|------|-------------|
  | `CONF_HIGH` | 引用了"可定位、有时间戳的公开数据" | 权重 ×1.0 |
  | `CONF_MEDIUM` | 有逻辑推断支撑但无直接可定位证据 | 权重 ×0.75 |
  | `CONF_LOW` | 主要靠 AI 训练知识推断，出现"众所周知""一般认为"等模糊措辞 | 权重 ×0.5 |
- **输出**：`step_confidence_summary`，标注每步的 CONF 级别 + 降权后的 Gate 分数
- ⚠️ **防偏离**：如果任何一步的 evidence_refs 为空或只有不可定位的描述，该步自动标记为 `CONF_LOW`

**Step9 未知→已知 Agent**：

- **AI 角色**：信息收集任务生成器
- **核心问题**：step6 列出的"不懂"，能不能变成"可追踪的待办"？
- **执行逻辑**：
  1. 对每条 uncertainty，判断是否"可解决"（需要什么信息？从哪获取？预计多久？）
  2. "可解决"的 → 创建 `info_collection_task`（数据源 + 采集方法 + deadline）
  3. "不可解决"的 → 标注 `IRRESOLVABLE`，写入 thesis 卡作为"需承受的风险"
  4. 对 `IRRESOLVABLE` 项给出"如果朝不利方向发展，何时应该退出"的判断
- **输出**：`uncertainties[]` 每项附加 `{task_created, resolvable, info_source, deadline, irresolvable_risk_response}`

##### §2.4.8.3 认知引擎五个设计缺陷与修正方案

> **本节为实施前必读**——防止执行模型按字面理解而偏离段永平投资哲学的真实意图。

**缺陷 1：线性分析 vs 全局直觉**

- **问题**：六步分析是线性拆解，段永平的思维是网状关联（他看 VCD"350 线"瞬间想到"用户在终端比的是肉眼效果不是数字"）
- **现象**：如果只跑六步线性分析，可能出现"每步都对，但整体判断错"。例如 step1 说"GPU 互联是真问题"，step6 说"不懂光通信底层"，但无法发现"不懂光通信的人，怎么确认自己真的理解了真问题？"
- **修正**：新增 **Step7 跨步关联 Agent**。具体做法：强制输出 ≥3 条跨步关联；关联模板覆盖 step1↔step3、step2↔step4、step1↔step6；每条关联必须回答"如果两步骤之间存在矛盾，这个生意还能投吗？"
- **执行注意**：跨步关联不是"总结六步分析"，是"发现内在矛盾"。0 矛盾输出 → 标记 `CROSS_STEP_SUSPICIOUS`（分析可能过浅或同构模型在相互奉承）

**缺陷 2：否定清单执行太晚**

- **问题**：原设计否定清单在认知引擎开始时执行，但此时已通过 Z1 Gate-A，浪费了认知成本
- **现象**：Z1 Gate-A 通过的标的中，20-30% 可能在否定清单中一票否决——这些不该进认知引擎
- **修正**：否定清单拆为两段——**前置否决**（Step0，Z1 Gate-A 通过后立即执行，检查硬性红线）+ **后置复验**（Step9 之后执行，检查段永平式软性判据）。前置否决通过才进 Step1-6，节省成本
- **执行注意**：前置否决必须严格执行，不得因"估值便宜"或"增长快"而跳过

**缺陷 3：step5 估值与 step_11 重复**

- **问题**：认知引擎自行做估值分析，与系统中已有的 step_11 估值动态评估器（纯数学计算、更可靠）重复
- **现象**：两个模块对同一标的估值结论可能不一致，且 AI 估值可能产生幻觉
- **修正**：Step5 **禁止**自行计算 PE/分位/Davis。纯数学结论**只来自 step_11**。Step5 的职责改为："基于 step_11 的数学结论 + step1-4 的认知结论，综合判断当前估值是否提供了足够安全边际"
- **执行注意**：Step5 的 prompt 必须显式声明 `DATA_SOURCE: step_11`，不得自行计算任何估值数字

**缺陷 4：认知引擎自身缺少置信度标记**

- **问题**：每步分析都是 AI 输出，但 AI 可能"言之凿凿地说错话"
- **现象**：如 AI 基于训练数据中的过时信息判断管理层诚信（如"某 CEO 被誉为行业良心"但近 2 年已行为变化），整个认知结论建立在错误基础上
- **修正**：新增 **Step8 置信度汇总**。每步标注 `CONF_HIGH/MEDIUM/LOW` 三级。`CONF_LOW` 的触发条件：出现"众所周知""通常""一般认为"等模糊措辞，或引用了无法定位出处的说法。在 Gate-A.5 中，`CONF_LOW` 的步骤按 0.5 折计算该步的 Gate 权重
- **执行注意**：置信度由 Step8 汇总 Agent 执行，检查每步输出的 evidence_refs 是否含"可定位引用"。空 evidence_refs → 自动 `CONF_LOW`

**缺陷 5："不懂"没有变成行动**

- **问题**：Step6 列出"还不懂的东西"，但列完就完了——系统没有自动去搞懂它
- **现象**：认知引擎完成 2 周后，step6 的 uncertainties 原封不动，无人跟进
- **修正**：新增 **Step9 未知→已知 Agent**。每条 uncertainty 自动判断是否"可解决"；可解决的创建 `info_collection_task`（数据源+采集方法+deadline），挂到 Z3 监控任务队列；不可解决的标注 `IRRESOLVABLE`，写入 thesis 卡作为"需承受的风险"并给出"不利发展时的退出条件"
- **执行注意**：Step9 不替代 Z3 的 13 轴探针——它只负责"把不知道变成追踪任务"

##### §2.4.8.4 Z1.5 与上下游的晋级契约

**Z1 → Z1.5 触发**：

| 触发条件 | 动作 |
|---------|------|
| Gate-A 通过 | 自动调度 cognition orchestrator；待办推送「机会雷达 · 可进入认知分析」 |
| Gate-A 未通过 | 不触发 Z1.5，标的停留在 Z1 |

**Z1.5 → Z2 晋级（消费 Gate-A.5 判定）**：

| Gate-A.5 结果 | 动作 |
|--------------|------|
| `pass` | 全额晋级 Z2；`BusinessCognitionPack` 注入 Z2 模板 A |
| `conditional_pass` | 晋级 Z2；但 thesis 卡标注认知边界追踪；模板 C 强制审查认知边界 |
| `fail` | 不晋级；标的退回 Z1 观察列表或丢弃 |

**Z1.5 → Z3 监控联动**：

| 认知引擎产出 | Z3 监控行为 |
|-----------|----------|
| step4.core_drivers 变化 | 触发 cognition orchestrator 重新运行 |
| step6.uncertainties[].task_created | Z3 监控任务到期提醒 |
| step6.verdict = conditional_pass | Z3 持续监控 uncertainties 的可解决性变化 |

<a id="design-32-gate-a_5"></a>

##### §2.4.8.5 Gate-A.5 · 认知深度验收闸门（Z1.5 → Z2）

> **目的**：确保标的在进入 Z2（买入论证台）之前，已经完成了足够的深度认知分析——不是"被打分了"而是"被理解了"。

**Gate-A.5 参数化**（`cognition_gates.yaml` · 禁止硬编码在代码）：

```yaml
gate_a_half:
  version: "1.0"
  calibration_status: provisional   # 待 Z4 季度复审校准
  step_requirements:
    step0_negation:
      required: true
      pass_condition: "overall_pass == true 且 veto_reasons is empty"
      fail_action: "DROP"
    step1_user_value:
      required: true
      irreplaceability_min: "medium"
      min_user_scenarios: 2
    step2_management:
      required: true
      integrity_score_min: 70
      consistency_score_min: 70
      overall_trust_min: "medium"
    step3_moat:
      required: true
      depth_score_min: 6
      min_advantages: 2
    step4_cashflow:
      required: true
      projections_complete: true    # 三档（悲观/中性/乐观）必须全部填写
    step5_valuation:
      required: true
      margin_of_safety_min_pct: 20
      data_source_verified: "step_11"  # 必须验证数据来源
    step6_boundary:
      required: true
      verdict_not: "fail"           # pass 或 conditional_pass
    step7_cross_step:
      required: true
      min_correlations: 3
      min_conflicts: 1              # 至少发现 1 条矛盾（无矛盾标记为 CROSS_STEP_SUSPICIOUS）
    step8_confidence:
      required: true
      no_step_conf_low_count_max: 2 # 最多允许 2 步为 CONF_LOW
    step9_unknown_to_known:
      required: true
      all_uncertainties_have_tasks: true  # 每条 uncertainty 必须有 info_collection_task
  overall_pass_condition: "ALL steps pass AND negation_checklist.overall_pass=true"
  conditional_pass_rules:
    - step: step6_boundary
      condition: "verdict == conditional_pass"
      action: "PASS_WITH_WARNING（进入 Z2 但 thesis 模板 C 必须审查认知边界）"
    - step: step2_management
      condition: "risk_signals non-empty"
      action: "PASS_WITH_WARNING（thesis P4 前提须强化监控风险信号）"
    - step: step1_user_value
      condition: "irreplaceability == low 且 step3 depth_score >= 6"
      action: "PASS_WITH_WARNING（高危：低不可替代但护城河深——可能漏判真正的竞争对手）"
  confidence_discount:
    CONF_HIGH: {weight_multiplier: 1.0}
    CONF_MEDIUM: {weight_multiplier: 0.75}
    CONF_LOW: {weight_multiplier: 0.5}
```

**Gate-A.5 准入判定逻辑**：

| Gate-A.5 结果 | 条件 | Z2 行为 |
|-------------|------|--------|
| **PASS** | 全体 step 通过 | 全额晋级 Z2；cognition_pack 注入 Z2 模板 A |
| **CONDITIONAL_PASS** | step6 为 conditional_pass 或 step2 有 risk_signals 或 step1 低 irreplaceability | 晋级 Z2 但标注；thesis 卡模板 C |
| **FAIL** | Step0 否决 / step6 verdict=fail / step8 ≥3 步 CONF_LOW | 标的退回 Z1 观察或丢弃；写入 Z4 跟踪 |

**执行约束**：
- Gate-A.5 由 `cognition orchestrator` 自动执行，无需人工确认
- 每次通过的 cognition_pack 写入 Z4 影子跟踪（与 thesis 同等待遇）
- `conditional_pass` 的 cognition 标的需要在 Z3 监控中持续跟踪 uncertainty 的变化

#### §2.4.9 节点级段永平过滤器（BOM 节点认可度）

> **定位**：Z0 战略板块 BOM（物料清单）节点层的段永平哲学质量标签。每个节点（如「智算中心土建与机柜」「光模块龙头」）获得一个段永平认可度评级，帮助架构师快速识别哪些产业链节点符合段永平的生意标准。

**哲学映射**：

| 段永平哲学维度 | 对应评分维度 | 权重 | 测量方法 |
|---------------|-------------|------|---------|
| "看不懂不投"、"伪龙头哨兵" | **D1 生意本质** | 30% | C7（行业分类引擎）pass rate 越高越好 |
| "护城河"、"长期确定性" | **D2 护城河深度** | 30% | 标的不可替代性（irreplaceability）平均值 |
| "好生意"、"长期" | **D3 长期确定性** | 20% | 节点层级核心度：核心=1.0 / 重要=0.7 / 配套=0.4 |
| "不做什么"、"利润之上的追求" | **D4 商业模式** | 20% | C5（避开肮脏赛道）bypass_risk 逆分 |

**评分算法**（`cvm_scorer.score_node_duan()`）：

```
aggregate = D1 × 0.30 + D2 × 0.30 + D3 × 0.20 + D4 × 0.20

D1 = (C7 pass 标的数) / 节点总标的数
D2 = avg(各标的 irreplaceability.score)   # 各标的可替代性
D3 = 核心→1.0, 重要→0.7, 配套→0.4
D4 = 1 - avg(各标的 C5.bypass_risk_normalized)
```

**标签阈值**：

| aggregate 区间 | 标签 | 含义 | 建议操作 |
|---------------|------|------|---------|
| ≥ 0.65 | ✅好生意 | 节点内在生意质量高，符合段永平标准 | 可放心深研 |
| 0.35 ~ 0.65 | ❓需深研 | 部分符合但有不确定性 | 需额外验证关键变量 |
| < 0.35 | ❌看不懂 | 生意复杂/护城河薄弱/商业模式不明 | 非核心持仓应跳过 |

**数据来源**：
- 主：`cvm_scorecards.scores_json`（C1~C7 评分 + irreplaceability + pool_gate）
- 备：`score_symbol()` 在线计算（纯 T1 规则，无 LLM 调用）
- 无数据时降级到仅 D3（节点层级）+ 中性预设值

**前端显示**：
- 每个 BOM 节点名称旁显示标签徽标（如 `✅好生意`）
- 鼠标悬浮显示四维评分详情（📊 图标触发 popover）
- 未来可扩展：按段永平认可度排序节点

**对应的代码实现**：
- `diting-src/apps/copilot/modules/strategic/cvm_scorer.py` :: `score_node_duan()`
- `diting-src/apps/copilot/modules/strategic/render.py` :: `_render_bom_stock_pool()`（duan_badge 徽标）
- `diting-src/apps/copilot/routers/strategic_z0_routes.py` :: `api_board_ecosystem_sorted()`（在线评分）

**实施要求**：
- **后端**：`cvm_scorer.py` 新增 `score_node_duan()` —— 【已完成】
- **渲染**：`render.py` 节点 header 首次渲染含段永平徽标 —— 【已完成】
- **路由**：`strategic_z0_routes.py` 首次渲染或排序时带 `duan_node_scores` 参数 —— 【已完成】
- **回填**：存量 board 的 BOM 节点标的 CVM 评分，可通过重新访问 `GET /api/strategic/boards/{board_id}/ecosystem/sorted` 自动触发在线评分（评分结果不落盘，仅在会话内使用）

**TODO（后续扩展）**：
- [ ] 节点排序选项增加「段永平认可度优先」
- [ ] 段永平标签条件可配置化（阈值 ±0.05 可调）
...
| **Z1→Z1.5** | 九维基线、待证伪假设 | 生意认知引擎 intake（cognition orchestrator 触发） |
| **Z1.5→Z2** | **`BusinessCognitionPack`**（否定清单 + 六步深度认知 + 跨步关联 + 置信度汇总 + 未知→已知） | Z2 证伪靶子 + 证据包起点 + thesis 卡 P0～Pn 证据引用 |
| **Z2→Z3** | 论文 P0～Pn、13 轴监控网、Kill 条件 | Z3 日更监护 |
| **Z3→Z4** | 预注册决策日志、前提快照 | Z4 影子对账 |
| **Z4→上游** | 探针 precision、尸检 | 修正 **Z0 CVM/E1～E5** / Z1 权重 / Z2 探针 |

**依赖顺序不可跳**：即使先买入（Z3），也必须 **Z0 → Z1 → Z1.5 → Z2 倒灌补齐** 后才能进入「正式持仓」监护。
...
> **Z1.5 本身不是数据采集器，而是九维分数+估值数据的深度认知加工器**。其消费已有指标、产出认知对象。

**消费指标**（Z1.5 执行前必须就位）：

| 消费来源 | 指标/对象 | 在认知引擎中使用 |
|---------|----------|---------------|
| Z1 Gate-A | `nine_dim_scorecard` | step1-6 前置数据读取（不重新打分） |
| Z2 evidence | `evidence_pack`（财务摘要） | step4 现金流推演引用 |
| step_11 | `ValuationDynamics` (davis_phase + pe_percentile + peg) | **step5 唯一数据来源**（禁止自行计算估值） |
| Z0 CVM | `cvm_scorecard` | step3 护城河分析语境 |
| Z0 dispatch | `scan_dispatch`（theme + phase_id + ecosystem） | step1 行业语境 |

**产出指标**（Z1.5 产出，供下游消费）：

| 产出对象 | 内容 | 下游消费方 |
|---------|------|----------|
| `business_cognition_pack.json` | 否定清单 + 六步深度认知 + 跨步关联 + 置信度汇总 + 未知→已知 | Z2 模板 A · Gate-A.5 · Z3 监控 · Z4 影子 |
| `gate_a_half_result.json` | Gate-A.5 判定结果（pass/conditional_pass/fail + 各步细项） | Z2 intake · 33_ 前端展示 |

**Z1.5 在 V0→V4 提炼链中的位置**：

| 层 | 认知引擎对应 |
|----|-----------|
| **V0-V1** | 不参与采集——全部消费既有 metric_store |
| **V2** | 不产生新的信号分位——消费既有 signal 层 |
| **V3（Value）** | ✨ **cognition_pack 属于 V3**：将九维分数+Davis 估值转化为对生意的深度价值判断 |
| **V4（Decision）** | Gate-A.5 判定 ∈ gates.yaml（与 Gate-A / Gate-B / Gate-C/D 同级） |
...
> **Z1.5 不是采集器**——其数据来源全部是上游既有指标的消费，不新增一手采集任务。

| 认知步 | 消费指标/对象 | 来源 Zone | 数据来源说明 |
|--------|------------|----------|------------|
| Step0 否定清单 | 财务造假记录、减持 signals、关联交易 | Z0 CVM/C5/C7 · Z2 evidence | 从既有指标读取，不重新采集 |
| Step1 用户价值 | D1（行业地位）、D3（成长空间）、scan_dispatch | Z1 · Z0 | 九维分数 + 战略板语境 |
| Step2 管理团队 | D7（治理——减持/关联交易）、交易所公告 | Z1 · Z2 evidence | 公告引擎已有数据 |
| Step3 护城河 | D2（技术壁垒）、cvm_scorecard、竞品比对 | Z0 · Z1 | 上游既有 |
| Step4 现金流 | evidence_pack（三表+附注）、巨潮完整年报 | Z2 evidence | 财报引擎已有数据 |
| Step5 估值 | **ValuationDynamics**（step_11） | 维度二 step_11 | ✨ 唯一估值来源，禁止自算 |
| Step6 认知边界 | 全部 Step1-5 输出 + uncertainties[] 自我审视 | Z1.5 内部自省 | 由 Step9 生成采集任务 |
| Step7 跨步关联 | Step1-6 全部输出 | Z1.5 内部推理 | 不消费新数据 |
| Step8 置信度 | Step1-7 输出 + evidence_refs 字段 | Z1.5 内部审计 | 检查引用可追溯性 |
| Step9 未知→已知 | Step6.uncertainties[] → info_collection_task | Z1.5 → Z3 监控队列 | 创建任务但不直接采集 |
...
**W1.5 生意认知引擎**：
- [ ] **BusinessCognitionPack Schema** 已定义（含 step0～step9 完整结构）
- [ ] `cognition_gates.yaml` 已登记 Gate-A.5（含 confidence_discount + conditional_pass_rules）
- [ ] **Step0 否定清单**已登记（≥12 条否决项，含硬性红线+段永平式软性判据）
- [ ] **Step5 估值**强制消费 `step_11`（prompt 显式声明 `DATA_SOURCE: step_11`，禁止自算）
- [ ] **Step7 跨步关联**已实现（输出 ≥3 条关联 + ≥1 条矛盾，0 矛盾标记 CROSS_STEP_SUSPICIOUS）
- [ ] **Step8 置信度**已实现（三级 CONF_HIGH/MEDIUM/LOW + Gate-A.5 降权逻辑）
- [ ] **Step9 未知→已知**已实现（uncertainties 自动生成 info_collection_task）
- [ ] `cognition orchestrator` 可实现 Z1 Gate-A 通过后自动触发
- [ ] Z2 模板 A 可消费 `business_cognition_pack.json`（thesis P0～Pn 证据引用带 cognition ref）
- [ ] Z3 监控队列可消费 cognition uncertainy 追踪任务
...
| 日期 | 版本 | 说明 |
|---|---|---|
| 2026-06-30 | **v4.1** | **节点级段永平过滤器（§2.4.9）**：BOM 节点四维评分（D1+C7 + D2+irreplaceability + D3+层级 + D4+C5）→ ✅好生意/❓需深研/❌看不懂 · 前端 node header 徽标 + 悬浮详情 · 在线评分兜底 |
| 2026-06-30 | **v4.0** | **生意认知引擎 Z1.5 全面集成**：§1.5 生意认知引擎概述 + 段永平 7 条哲学映射；§2.1 漏斗图新增 Z1.5 节点 + §2.2 速查表新增 Z1.5 行；**§2.4.8 九步认知完整工作流**（Step0 否定清单→Step1 用户价值→Step2 管理团队→Step3 护城河→Step4 现金流→Step5 估值（消费 step_11）→Step6 认知边界→Step7 跨步关联→Step8 置信度汇总→Step9 未知→已知）；**§2.4.8.3 认知引擎五个设计缺陷与修正方案**；**§2.4.8.5 Gate-A.5** 参数化（cognition_gates.yaml · 含 confidence_discount + conditional_pass_rules）；§2.6 交付物 DAG 更新；§5A 新增 Z1.5 认知引擎指标线（V3 落位）；§6 新增 §6.3 生意认知坊数据指标；§14 新增 W1.5 认知引擎验收项；与 [43_](./43_五区工作流设计合规性验证矩阵.md) 校验文档同步 |
