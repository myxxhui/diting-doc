...
```
你是在 diting-core 中执行 Stage2-03（本地测试与 K3s 连调）的实践者。必读：03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md、03_原子目标与规约/_共享规约/05_接口抽象层规约.md（MarketDataFeed）。
...
| 6 | **diting-core** 根目录 | （可选）验证 MarketDataFeed 读 L1：运行使用 MarketDataFeed 的脚本或单测（见 05_接口抽象层规约） | 能从 L1 读取 OHLCV 数据 |
| 7 | **diting-core** 根目录 | （可选）按 [02_采集逻辑与Dockerfile#目标数据约定与真实结果](02_采集逻辑与Dockerfile.md#l4-stage2-02-target-data) 执行 5 条 psql 验证查询，确认 symbol、period、L2 data_type 与约定一致 | 与 docs/ingest-test-target.md 约定一致 |
| 8 | **diting-infra** 根目录 | 回收环境：`make local-deps-down` | 本地 L1/L2 容器停止并移除 |
...
适用于需要验证与**真实 K3s 集群、NodePort、Sealed-Secrets** 一致时；L1/L2 在 **diting-infra** 部署的 K3s 上，**diting-core** 在本地通过 NodePort/节点 IP 连接。前置条件： [01_基础设施与依赖部署](01_基础设施与依赖部署.md) 已准出（V1～V7），或已按该文档在 diting-infra 完成部署。
...
| 6 | **diting-core** 根目录 | （可选）验证 MarketDataFeed 读 L1：使用 TimescaleDBFeed 等从 L1 读 OHLCV | 能从远程 L1 读取数据 |
| 7 | **diting-infra** / **diting-core** | 验证数据落库：在集群内 `kubectl exec` 执行 psql，或在 diting-core 用 `psql $TIMESCALE_DSN -c "SELECT count(*) FROM ohlcv"` 等 | 能查到本次 ingest-test 写入的数据 |
| 8 | **diting-infra** | **回收**（若为临时验证环境）：按 [01_基础设施与依赖部署#本步实践总结](01_基础设施与依赖部署.md#l4-stage2-01-summary) 执行 `make stage2-01-down` 与 `make down`（或 `make stage2-01-full-down`） | 无 ECS/K3s 残留，避免继续计费 |
...
- **本地 Docker Compose**：完成「最佳实践一」中步骤 1～5（或 `make local-practice` + 5 条 psql），且 MarketDataFeed 可读 L1、make ingest-test 通过。
- **远程 K3s**：完成「最佳实践二」中步骤 1～5（或 1～7），且本地进程可写远程 L1、MarketDataFeed 可读远程 L1、make ingest-test 通过。

准出后**已更新 L5 [02_验收标准 对应行](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_03)**。
