...
```
你是在 diting-core 中执行 Stage2-02（采集逻辑与 Dockerfile）的实践者。必读：11_数据采集与输入层规约、global_const.data_ingestion、07_数据版本控制规约。

任务：
1. 实现采集任务（ingest_ohlcv、ingest_industry_revenue、ingest_news），按 11_ 规约写入 L1/L2。
