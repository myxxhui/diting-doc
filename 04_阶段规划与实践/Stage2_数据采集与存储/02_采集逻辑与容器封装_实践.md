...
> - **本步设计文档**: [02_采集逻辑与容器封装_设计](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-exit)
> - **本步 DNA 文件**: [dna_stage2_02_采集逻辑与Dockerfile.yaml](../../03_原子目标与规约/_System_DNA/Stage2_数据采集与存储/dna_stage2_02_采集逻辑与Dockerfile.yaml)
> - **逻辑填充期接入点**：本步须按设计文档中「逻辑填充期开源接入点」小节实现并达标，见 [AkShare](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-integration-akshare)、[OpenBB](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-integration-openbb)。**右脑数据支撑**：F11 **ingest_business_profile**（AkShare `stock_zygc_em`）→ L2 **segment_registry** / **symbol_business_profile**，见 [12_右脑数据支撑与Segment规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md)；生产流水线见 Stage3 [01_内容理解与语义分类_实践](../Stage3_模块实践/01_内容理解与语义分类_实践.md) Phase 2.3。

<a id="l4-step-nav"></a>
## 步骤导航
- **上一步**：[01_数据底座与基础依赖_实践](01_数据底座与基础依赖_实践.md#l4-stage2-01-goal)
...
本步依赖 [01_数据底座与基础依赖_实践](01_数据底座与基础依赖_实践.md#l4-stage2-01-goal) 产出的数据库与连接方式。Stage2-01 准出时已包含**下游如何添加数据库连接配置并调用表数据**的验证；本步须与下列约定一致，并可作为 Stage2-01 的「下游引用示例」验证执行方。

| 依赖项 | 说明 | 本步用法 |
|--------|------|----------|
| **数据库连接配置** | 由 Sealed-Secrets 或 .env 提供；占位项见 diting-core `.env.template`（如 `TIMESCALE_DSN`、`REDIS_URL`、`PG_L2_DSN`），与 diting-infra 部署的 Service/NodePort 对应 | 采集任务写入 L1 TimescaleDB、L2 知识库时使用上述 DSN；配置来源见 [01_基础设施与依赖实践#关键下游引用与验证要求](01_数据底座与基础依赖_实践.md#l4-stage2-01-downstream) |
...
| **示例与验证** | Stage2-01 文档中「关键下游引用与验证要求」要求：在 diting-core 中提供最小验证（如 `make verify-db-connection`），连接 DB 并对 init 所建表执行 SELECT，退出码 0 | 本步实现时须提供该 make target 或等价脚本，供 Stage2-01 准出时执行；实现方式见 [01_数据底座与基础依赖_设计](../../03_原子目标与规约/Stage2_数据采集与存储/01_基础设施与依赖_设计.md#design-stage2-01-exit) 与 11_ 规约 |
...
本步依赖 [01_数据底座与基础依赖_实践](01_数据底座与基础依赖_实践.md#l4-stage2-01-exit) 的验证项 V1～V7 全部通过。执行本步前建议逐项核对：
...
| 顺序 | 执行位置 | 执行内容 | 说明 |
|------|----------|----------|------|
| ① | **diting-infra** | 若无可用集群：执行 `make deploy-dev`，等待完成；根据 deploy-engine 输出设置 **KUBECONFIG** | 需 Terraform/云凭证；与 [01_数据底座与基础依赖_实践](01_数据底座与基础依赖_实践.md)、[03_基础设施ECS与K3s就绪](../../Stage1_仓库与骨架/03_基础设施ECS与K3s_实践.md) 一致 |
| ② | **diting-infra** | V1 校验：`test -d charts/dependencies/timescaledb && grep -E '^version:' charts/dependencies/timescaledb/Chart.yaml`（redis、postgresql 同理）；确认 charts/values/*.yaml 存在且镜像 tag 已固定 | 见 [01_基础设施与依赖实践 步骤 1](01_数据底座与基础依赖_实践.md#l4-stage2-01-summary) |
...
**权威引用**：①～④ 与 [01_数据底座与基础依赖_实践](01_数据底座与基础依赖_实践.md#l4-stage2-01-summary)、diting-infra [Stage2-01-部署与验证](../../../diting-infra/docs/Stage2-01-部署与验证.md) 一致；回收与 [清除验证环境（必做)](../../../diting-infra/docs/Stage2-01-部署与验证.md) 一致。
...
按 [11_ 数据采集与输入层规约](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md) 与 [03_双目标系统与五层架构](../../01_顶层概念/03_双目标系统与五层架构.md)，本步 F1～F8 覆盖**全链路研究**所需数据采集能力（含 Segment / 中长期候选链所在表的入仓约定）。**生产级**必采清单、存储表与验收见 [11_ 数据采集与输入层规约](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md)，对应实践见 [06_生产级数据要求_实践](../Stage2_数据采集与存储/06_生产级数据要求_实践.md)。
...
下表列出本步**全部功能/交付项**；执行者须逐项标注「实践状态」与「说明」，便于区分**已实践 / 未实践**。与设计文档 [功能项与验收映射](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-feature-mapping)、DNA `feature_items` 一致。

| 功能项 ID | 功能描述 | 对应设计/DNA | 实践状态 | 说明/代码位置 |
|-----------|----------|--------------|----------|----------------|
| F1 | ingest_ohlcv：AkShare A 股日线 → L1 ohlcv | [设计-AkShare](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-integration-akshare)、artifacts | ✅ 已实现 | diting-core/diting/ingestion/ohlcv.py |
| F2 | ingest_industry_revenue：AkShare 行业/财报/营收 → L2 或约定存储 | 同上 | ✅ 已实现 | diting-core/diting/ingestion/industry_revenue.py |
| F3 | ingest_news：AkShare 国内 + OpenBB 国际 → L2 data_versions | [设计-OpenBB](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-integration-openbb) | ✅ 已实现 | diting-core/diting/ingestion/news.py |
...
| F9 | 一键构建所有镜像：Makefile 提供单一 target（如 `make build-images`），一次构建本阶段所涉全部镜像 | [设计-依赖与镜像构建](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-deps) | ✅ 已实现 | diting-core/Makefile 中 build-images |
| F10 | 全 A 股标的池（universe）表 + 更新 Job；get_current_a_share_universe() 可读 | [11_ 全 A 股标的池](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe)、[设计 F10](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与容器封装_设计.md#design-stage2-02-feature-mapping) | 待实现/已实现 | universe 表与更新 Job；表名、字段、有效条件与 11_ 一致 |
