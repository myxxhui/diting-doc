...
| F10 | 全 A 股标的池（universe）表 + 更新 Job；get_current_a_share_universe() 可读 | [11_ 全 A 股标的池](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe)、[设计 F10](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile_设计.md#design-stage2-02-feature-mapping) | 待实现/已实现 | universe 表与更新 Job；表名、字段、有效条件与 11_ 一致 |

**填写约定**：实践状态三选一；说明/代码位置填仓库路径或简短结论。准出时 F1～F10 须均为「✅ 已实现」。
