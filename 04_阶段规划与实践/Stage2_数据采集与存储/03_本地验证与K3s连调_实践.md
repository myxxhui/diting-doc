...
| 2 | **diting-infra** | 获取连接串：从 Secret `diting-db-connection` 或部署文档中取得 `TIMESCALE_DSN`、`PG_L2_DSN`、`REDIS_URL`（均必填）；host 填 **K3s 节点 IP**（或可访问的 LB 地址），port 填 **NodePort** | 得到完整 DSN，格式如 `postgresql://user:****@<节点IP>:<NodePort>/dbname`、`redis://...` |
