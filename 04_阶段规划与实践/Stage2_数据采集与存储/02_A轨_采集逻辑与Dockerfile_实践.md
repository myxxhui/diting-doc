...
**结论**：完成本步 F1～F10 并通过 V-DB/V-INGEST/V-DATA/V-IMAGE/V-BUILD-ALL，即可保证采集层产出满足系统与双轨目标判断所需**生产级**数据：数据类型、数据量（单标日线 ≥5 年、标的 = universe）、universe 表与 `get_current_a_share_universe()`、复权口径与 06_/11_ 一致；验收可辅以 [06_生产级数据要求_实践](06_生产级数据要求_实践.md) 中的 `make verify-data-production`（若已实现）。
...
**准出**：① F1～F10 均为「✅ 已实现」；② 上表五行测试结果均为「通过」；③ [目标数据约定与真实结果](#l4-stage2-02-target-data) 中真实数据满足**生产级**（单标日线 ≥5 年、标的 = universe、复权一致）并已填写；④ 可选：diting-core 实现 `make verify-data-production` 且通过（见 [06_生产级数据要求_实践](06_生产级数据要求_实践.md)）。满足后更新 L5 [02_验收标准 对应行](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_02)。
...
| F11 | ingest_business_profile（可选）：标的主营业务构成 → L2 标的主营构成表；细分注册表初始化 | [12_右脑数据支撑与细分规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md) | 可选 | 表 DDL 与写入逻辑见 12_；细分标识仅用细分注册表中已注册值 |

**填写约定**：实践状态三选一；说明/代码位置填仓库路径或简短结论。准出时 F1～F10 须均为「✅ 已实现」；F11 为可选（右脑数据支撑）。
...
**准出条件**：见 [验证项与结果清单](#l4-stage2-02-verify-checklist) 节末（F1～F10 已实现、五验证项通过、目标数据满足生产级并已填）。
