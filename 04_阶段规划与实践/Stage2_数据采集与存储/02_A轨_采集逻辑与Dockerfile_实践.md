...
> - **本步设计文档**: [02_A轨_采集逻辑与Dockerfile_设计](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-exit)
> - **本步 DNA 文件**: [dna_stage2_02_采集逻辑与Dockerfile.yaml](../../03_原子目标与规约/_System_DNA/Stage2_数据采集与存储/dna_stage2_02_采集逻辑与Dockerfile.yaml)
> - **逻辑填充期接入点**：本步须按设计文档中「逻辑填充期开源接入点」小节实现并达标，见 [AkShare](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-integration-akshare)、[OpenBB](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-integration-openbb)。**右脑数据支撑**：F11 **ingest_business_profile**（AkShare `stock_zygc_em`）→ L2 **segment_registry** / **symbol_business_profile**，见 [12_右脑数据支撑与Segment规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md)；生产流水线见 Stage3 [01_A轨_语义分类器_实践](../Stage3_模块实践/01_A轨_语义分类器_实践.md) Phase 2.3。

<a id="l4-step-nav"></a>
## 步骤导航
- **上一步**：[01_A轨_基础设施与依赖_实践](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-goal)
...
**上下游关系总览**：本步**消耗** Stage2-01 准出（数据库与连接配置、表已建、`make verify-db-connection` 可过）；**产出** 采集任务（ingest_ohlcv、ingest_industry_revenue、ingest_news、**ingest_business_profile**）、Dockerfile 与 `make ingest-test`，供 Stage2-03（本地测试与 K3s 连调）、Stage2-04/05（镜像打包与采集模块部署）与 Stage3 Module A（真实 segment_shares）使用。

## 关键下游依赖（来自 Stage2-01）

本步依赖 [01_A轨_基础设施与依赖_实践](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-goal) 产出的数据库与连接方式。Stage2-01 准出时已包含**下游如何添加数据库连接配置并调用表数据**的验证；本步须与下列约定一致，并可作为 Stage2-01 的「下游引用示例」验证执行方。

| 依赖项 | 说明 | 本步用法 |
|--------|------|----------|
| **数据库连接配置** | 由 Sealed-Secrets 或 .env 提供；占位项见 diting-core `.env.template`（如 `TIMESCALE_DSN`、`REDIS_URL`、`PG_L2_DSN`），与 diting-infra 部署的 Service/NodePort 对应 | 采集任务写入 L1 TimescaleDB、L2 知识库时使用上述 DSN；配置来源见 [01_基础设施与依赖实践#关键下游引用与验证要求](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-downstream) |
...
| **示例与验证** | Stage2-01 文档中「关键下游引用与验证要求」要求：在 diting-core 中提供最小验证（如 `make verify-db-connection`），连接 DB 并对 init 所建表执行 SELECT，退出码 0 | 本步实现时须提供该 make target 或等价脚本，供 Stage2-01 准出时执行；实现方式见 [01_A轨_基础设施与依赖_设计](../../03_原子目标与规约/Stage2_数据采集与存储/01_基础设施与依赖_设计.md#design-stage2-01-exit) 与 11_ 规约 |
...
| **可选：生产数据环境** | 在 **diting-infra** 执行 `make deploy diting prod`，取得 **`prod.conn`**；在 **diting-core** 从该文件取 DSN 填入 `.env`，执行 `make verify-db-connection` → `make ingest-test`。后半段时由 Up 脚本在集群就绪后**内调 make ingest-test** 落库。回收用 `make down diting prod`（仅回收 ECS/K3s/EIP，**不删数据盘**）。与 [06_生产级数据要求_实践](06_生产级数据要求_实践.md) 前半/后半验证衔接。 | 生产级数据验收、全量 ingest 与数据继承验证；见 06_ 实践「生产数据环境」小节。 |

**填写约定**：在 [验证项与结果清单](#l4-stage2-02-verify-checklist)、[本步实践总结](#l4-stage2-02-summary) 的备注或「是否符合预期」中注明本次采用的路线（如「默认 Docker Compose」「可选 ECS+K3s」或「可选 生产数据环境」），便于追溯。**外网不可达时**：可设置 `DITING_INGEST_MOCK=1` 后执行 `make ingest-test`（及镜像内同命令），采集逻辑会写入 mock 数据至 L1/L2，仍可完成 V-INGEST、V-DATA、V-IMAGE 验证。
...
- **可选路线（生产数据环境）**：在 diting-infra 执行 `make deploy diting prod` 后，从 **`prod.conn`** 取 TIMESCALE_DSN、PG_L2_DSN、REDIS_URL 填入 diting-core 的 `.env`，再执行 `make verify diting prod`、`make ingest-test`；后半段时 Up 脚本内会调 make ingest-test 落库。回收用 `make down diting prod`，数据保留在盘上。与 [06_生产级数据要求_实践](06_生产级数据要求_实践.md) 后半验证衔接。

### 1. 依赖部署步骤检查清单

本步依赖 [01_A轨_基础设施与依赖_实践](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-exit) 的验证项 V1～V7 全部通过。执行本步前建议逐项核对：

| 检查项 | 说明 | 本步用途 |
|--------|------|----------|
| V1 | Chart/中间件版本已固定并缓存 | 无需本步操作，仅确认 Stage2-01 已做 |
| V2 | TimescaleDB 部署并可用 | 采集任务写入 L1 表 `ohlcv` |
| V3 | Redis 部署并可用 | **默认部署且必验**（缓存/队列）；`make verify diting prod` 含 Redis 校验与简单性能测试；按 11_ 规约 |
...
| **REDIS_URL** | Redis 连接串（**必填**；默认部署且必验，见 11_ 规约） | 同上；values 或 Secret 中的 auth.password 与 host/port 拼成 | `redis://:****@<host>:6379/0` |
...
3. **填写 .env**：将 Stage2-01 部署得到的 `TIMESCALE_DSN`、`PG_L2_DSN`、`REDIS_URL` 填入（必填；可从 diting-infra 的 Secret `diting-db-connection` 或部署文档中的示例/NodePort 拼接得到；本地连 K3s 时 host 为 NodePort 对应地址）。
...
| 顺序 | 执行位置 | 执行内容 | 说明 |
|------|----------|----------|------|
| ① | **diting-infra** | 若无可用集群：执行 `make deploy-dev`，等待完成；根据 deploy-engine 输出设置 **KUBECONFIG** | 需 Terraform/云凭证；与 [01_A轨_基础设施与依赖_实践](01_A轨_基础设施与依赖_实践.md)、[03_基础设施ECS与K3s就绪](../../Stage1_仓库与骨架/03_基础设施ECS与K3s_实践.md) 一致 |
| ② | **diting-infra** | V1 校验：`test -d charts/dependencies/timescaledb && grep -E '^version:' charts/dependencies/timescaledb/Chart.yaml`（redis、postgresql 同理）；确认 charts/values/*.yaml 存在且镜像 tag 已固定 | 见 [01_基础设施与依赖实践 步骤 1](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-summary) |
...
| ⑤ | **diting-core** | 复制 `.env.template` 为 `.env`，填入 TIMESCALE_DSN、PG_L2_DSN、REDIS_URL；执行 `make verify diting prod`，退出码 0 | 确认下游可连 L1/L2/Redis，且 Redis 缓存/队列性能测试通过 |
...
**权威引用**：①～④ 与 [01_A轨_基础设施与依赖_实践](01_A轨_基础设施与依赖_实践.md#l4-stage2-01-summary)、diting-infra [Stage2-01-部署与验证](../../../diting-infra/docs/Stage2-01-部署与验证.md) 一致；回收与 [清除验证环境（必做)](../../../diting-infra/docs/Stage2-01-部署与验证.md) 一致。
...
OHLCV/新闻/行业 全数据结构与逻辑完整；Dockerfile 支持采集任务构建。**数据量与范围按生产级要求**：单标的日线历史深度 ≥5 年、标的覆盖与全 A 股 universe 一致、复权与回测/实盘一致（见 [06_生产级数据要求_设计](../../03_原子目标与规约/Stage2_数据采集与存储/06_生产级数据要求_设计.md)、[11_ 生产级数据要求](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md)）；universe 表与更新 Job 必做，供 `get_current_a_share_universe()` 读取。采集镜像须在 Dockerfile/requirements 中显式安装 AkShare、OpenBB（见设计文档「[依赖与镜像构建](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile_设计.md#design-stage2-02-deps)」与 dna_stage2_02.integration_packages）。**本步须提供一键构建本阶段所涉全部镜像的能力**：Makefile 提供单一 target（如 `make build-images`），一次执行即可构建本阶段全部镜像（当前为采集镜像 Dockerfile.ingest），便于 CI 与本地复现。
...
| **ingest_business_profile** | AkShare `stock_zygc_em`（东方财富主营构成，按产品） | L2 `segment_registry` + `symbol_business_profile`；Module A **segment_shares** 优先数据源 | [12_右脑数据支撑与Segment规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md) §5.2 ；建表 `make init-l2-business-profile-tables` |
...
- **L2 表结构**：`data_versions(data_type, version_id, timestamp, file_path, ...)` 等，见 diting-infra `schemas/sql/02_l2_data_versions.sql`；新闻/知识库写入 Agri-KG、Tech-KG、Macro-KG 按 11_ 与 07_ 规约。**主营构成**：`09_l2_segment_registry.sql`、`10_l2_symbol_business_profile.sql`；采集实现 `diting/ingestion/business_profile.py`。

### 2. 实践方式与依赖组件

- **国内数据**：以 **AkShare** 为统一 Python 接口；`ingest_ohlcv`（A 股）、`ingest_industry_revenue`、`ingest_business_profile`（主营构成）、`ingest_news`（国内部分）必须走 AkShare，接口边界、错误与限流、写入契约见设计文档 [逻辑填充期开源接入点：AkShare](../../03_原子目标与规约/Stage2_数据采集与存储/02_采集逻辑与Dockerfile_设计.md#design-stage2-02-integration-akshare)。
...
按 [11_ 数据采集与输入层规约](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md) 与 [03_双轨制与VC-Agent](../../01_顶层概念/03_双轨制与VC-Agent.md)，本步 F1～F8 覆盖系统（含 A/B 双轨）所需数据采集能力。**B 轨**必采清单与存储表见 [03_/B轨/02_B轨数据与存储规约](../../03_原子目标与规约/B轨/02_B轨数据与存储规约.md)，实践见 [04_/B轨/01_B轨数据与采集_实践](../B轨/01_B轨数据与采集_实践.md)。
...
| **双轨共用**：主营构成分部披露供 Module A **segment_shares**（右脑） | F11 `ingest_business_profile` | AkShare `stock_zygc_em` → L2 `segment_registry` / `symbol_business_profile`；无披露时 A 轨单条领域回退，见 12_ §5.2 |
...
| **数据量与范围** | F8、F10、`docs/ingest-test-target.md` | **生产级**：单标日线 ≥5 年、标的 = 全 A 股 universe、复权与 Stage4 一致；与 [06_生产级数据要求](06_生产级数据要求_实践.md)、[Stage2 README#数据采集规划与实践步骤对照](README.md#stage2-data-plan-vs-steps) 一致 |

**结论**：完成本步 F1～F11 并通过 V-DB/V-INGEST/V-DATA/V-IMAGE/V-BUILD-ALL，即可保证采集层产出满足系统与双轨目标判断所需**生产级**数据：数据类型、数据量（单标日线 ≥5 年、标的 = universe）、universe 表与 `get_current_a_share_universe()`、复权口径与 06_/11_ 一致；含主营构成（F11）供 Module A **segment_shares**；验收可辅以 [06_生产级数据要求_实践](06_生产级数据要求_实践.md) 中的 `make verify-data-production`（若已实现）。
...
任务：
1. 实现采集任务（ingest_ohlcv、ingest_industry_revenue、ingest_news、**ingest_business_profile**），按 03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md 规约写入 L1/L2；主营构成表须先 `make init-l2-business-profile-tables`（见 12_ §5.2）。
...
**准出**：① F1～F11 均为「✅ 已实现」；② 上表五行测试结果均为「通过」；③ [目标数据约定与真实结果](#l4-stage2-02-target-data) 中真实数据满足**生产级**（单标日线 ≥5 年、标的 = universe、复权一致）并已填写；④ 可选：diting-core 实现 `make verify-data-production` 且通过（见 [06_生产级数据要求_实践](06_生产级数据要求_实践.md)）。满足后更新 L5 [02_验收标准 对应行](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage2_02)。
...
**约定内容（生产级）**：与 [06_生产级数据要求_设计](../../03_原子目标与规约/Stage2_数据采集与存储/06_生产级数据要求_设计.md)、[11_ 生产级数据要求](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md) 一致。L1：标的范围 = 全 A 股（universe 表或 `get_current_a_share_universe()` 一致）、单标的日线历史深度 ≥5 年、period（如 daily）、复权口径与 Stage4 一致；L2：data_type（如 industry_revenue、news）及预期条数；universe 表可查、有效条件与 11_ 一致。
...
下表列出本步**全部功能/交付项**；执行者须逐项标注「实践状态」与「说明」，便于区分**已实践 / 未实践**。与设计文档 [功能项与验收映射](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-feature-mapping)、DNA `feature_items` 一致。

| 功能项 ID | 功能描述 | 对应设计/DNA | 实践状态 | 说明/代码位置 |
|-----------|----------|--------------|----------|----------------|
| F1 | ingest_ohlcv：AkShare A 股日线 → L1 ohlcv | [设计-AkShare](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-integration-akshare)、artifacts | ✅ 已实现 | diting-core/diting/ingestion/ohlcv.py |
| F2 | ingest_industry_revenue：AkShare 行业/财报/营收 → L2 或约定存储 | 同上 | ✅ 已实现 | diting-core/diting/ingestion/industry_revenue.py |
| F3 | ingest_news：AkShare 国内 + OpenBB 国际 → L2 data_versions | [设计-OpenBB](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-integration-openbb) | ✅ 已实现 | diting-core/diting/ingestion/news.py |
...
| F7 | Dockerfile/requirements 显式 akshare、openbb-platform | dna_stage2_02.integration_packages | ✅ 已实现 | Dockerfile.ingest、requirements-ingest-core.txt；镜像与本地 `make deps-ingest` 一致：akshare==1.17.1 --no-deps + requirements-ingest-core，满足 Python≥3.8 且避免 curl_cffi 0.13 依赖问题 |
| F8 | ingest-test 目标数据约定（生产级：单标日线 ≥5 年、标的 = universe、复权一致） | [目标数据约定与真实结果](#l4-stage2-02-target-data)、[06_生产级数据要求](06_生产级数据要求_实践.md) | ✅ 已实现 | diting-core/docs/ingest-test-target.md（须满足 06_/11_ 生产级） |
| F9 | 一键构建所有镜像：Makefile 提供单一 target（如 `make build-images`），一次构建本阶段所涉全部镜像 | [设计-依赖与镜像构建](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-deps) | ✅ 已实现 | diting-core/Makefile 中 build-images |
| F10 | 全 A 股标的池（universe）表 + 更新 Job；get_current_a_share_universe() 可读 | [11_ 全 A 股标的池](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md#全-a-股标的池universe)、[设计 F10](../../03_原子目标与规约/Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_设计.md#design-stage2-02-feature-mapping) | 待实现/已实现 | universe 表与更新 Job；表名、字段、有效条件与 11_ 一致 |
| F11 | ingest_business_profile：AkShare 主营构成（按产品）→ L2 **segment_registry** + **symbol_business_profile**；稳定 `seg_bp_*`；生产流水线见 `run_ingest_production.py` Phase 2.3 | [12_右脑数据支撑与Segment规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md) §5.2 | ✅ 已实现 | `diting/ingestion/business_profile.py`；`diting-infra/schemas/sql/09_*.sql`、`10_*.sql`；`make init-l2-business-profile-tables` |

**填写约定**：实践状态三选一；说明/代码位置填仓库路径或简短结论。准出时 F1～F11 须均为「✅ 已实现」（右脑数据支撑为生产级 Module A 必备路径）。
...
**准出条件**：见 [验证项与结果清单](#l4-stage2-02-verify-checklist) 节末（F1～F11 已实现、五验证项通过、目标数据满足生产级并已填）。
...
| 2 | 在 diting-core 复制 .env.template 为 .env，填写 TIMESCALE_DSN、PG_L2_DSN、REDIS_URL；执行 `make verify diting prod` | 退出码 0；能连接 L1/L2 并查询 init 所建表，Redis 连通且缓存/队列性能测试通过 | V-DB、V3 | 是 |
