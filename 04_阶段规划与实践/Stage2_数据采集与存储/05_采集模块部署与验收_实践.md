...
| 1 | **diting-infra** | 部署采集模块 | 使用 **K3s Job**：`make deploy-ingest-job [WAIT=wait]`（Secret 来自 prod.conn，镜像与 target 见 config；与 [06_生产级数据要求_实践](06_生产级数据要求_实践.md#l4-stage2-06-k3s-and-image) 一致） | 与 [01_基础设施与依赖_实践](01_基础设施与依赖_实践.md) 一致 |
| 2 | **diting-infra** | 触发并等待采集完成 | Job **diting-ingest** 在 K3s 内执行 `make <target>`，`WAIT=wait` 时等待 Job 完成 | 采集写入 L1/L2 |
...
- **生产部署**：调度频率与超时须与 [11_ 数据采集与输入层规约](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md) 及 DNA `data_ingestion`（如 `schedule: daily/hourly`、`timeout_minutes`）一致；cron/Job 配置在部署侧完成。数据量与范围：自 02 步起已按生产级（单标日线 ≥5 年、标的 = universe、复权一致），见 [Stage2 README#数据采集规划与实践步骤对照](README.md#stage2-data-plan-vs-steps)、[06_生产级数据要求_实践](06_生产级数据要求_实践.md)。
