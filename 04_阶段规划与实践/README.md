...
**示例**：[Stage1-01 三位一体仓库初始化](Stage1_仓库与骨架/01_三位一体仓库_实践.md)、[Stage2-02 采集逻辑与Dockerfile](Stage2_数据采集与存储/02_采集逻辑与Dockerfile_实践.md) 为核心指令由 DNA 键展开的参考写法。

### 实践步骤验证与准出参考（Stage2-01 案例）

**其它所有实践步骤**宜参考 [Stage2-01 基础设施与依赖部署](Stage2_数据采集与存储/01_基础设施与依赖_实践.md) 的以下约定，保持可执行、可评判、下游可追溯：

| 约定 | 要求 | 参考位置 |
|------|------|----------|
| **版本固定与缓存** | Chart、中间件等部署文件必须版本固定并缓存（如 charts/dependencies/ 本地存放）；禁止部署时从远程动态拉取未固定版本；code 逻辑视情况固定 | [01_基础设施与依赖_实践#版本固定与缓存约定](Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-version-pinning)、DNA `pinned_versions` |
| **验证三项** | ① 列出验证项 ② 列出验证方式与预期目标 ③ 评判验证结果是否符合预期（符合/不符合）；全部符合方可准出 | [01_基础设施与依赖_实践#验证与准出](Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-exit) |
| **下游关键引用与核心验证** | 若有下游依赖，须有关键下游引用示例（配置如何添加、如何调用本步产出）；示例须可执行，且作为本步准出核心验证指标之一；全部验证通过才认为本步验证通过 | [01_基础设施与依赖_实践#关键下游引用与验证要求](Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-downstream)；下游步骤文档须含「关键下游依赖」并注明验证归属 |
