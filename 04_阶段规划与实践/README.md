...
> **需求/变更入口**：[四阶段提问体系 完整规约](00_四阶段提问体系_完整规约.md) — 将新需求/设计变更通过四阶段（需求表达→技术方案与文档映射→批判补充→文档操作与实践清单）转化为待办清单，审查后按实践任务执行或 build。
...
**人类执行者与 1:1:1 核对**：人类在执行修复或采纳新需求/设计变更时，应遵循 [00_系统规则 §4.6、§4.7](../00_系统规则_通用项目协议.md#46-debug修复疑问时的-111-检查与问题穿透防护) 的 1:1:1 检查与反写要求（修复后核对 03_/DNA/04_/L5；新需求或设计变更当次反写）；设计–DNA–实践–L5 索引见 [03_原子目标与规约/README](../03_原子目标与规约/README.md)。
...
**三轨与实现优先序**：系统为 **A 轨（短线）、B 轨（中线）、C 轨（长线）** 三轨制；**设计与开发优先实现 B 轨（中线捕手轨）**，目标 1～2 月 30%～100%+、年复利 200%。**B 轨专项实践**（与 A 轨同列各 Stage）：Stage2 [01_B轨_数据与采集_实践](Stage2_数据采集与存储/01_B轨_数据与采集_实践.md)、Stage3 [01_B轨_语义与候选_实践](Stage3_模块实践/01_B轨_语义与候选_实践.md)、Stage4 [01_B轨_共识与执行_实践](Stage4_MoE与执行网关/01_B轨_共识与执行_实践.md)；设计见 [03_/B轨/](../03_原子目标与规约/B轨/README.md)（含 [00_B轨整体设计](../03_原子目标与规约/B轨/00_B轨整体设计.md)）。**C 轨（长线）** 实践在 B 轨落地后扩展，设计见 [03_/C轨/](../03_原子目标与规约/C轨/README.md)。
...
**核心指令 DNA 驱动约定**：核心指令须包含「读取本阶段 DNA 的 `delivery_scope`、`exit_criteria`、`artifacts`、`verification_commands` → 据此生成任务列表与验收项」；实施与验收不得偏离 DNA，冲突以 DNA 为准。**核心指令内引用须 AI 可读**：对文档、DNA、L5 的引用须为完整路径、DNA 键路径或步骤内「本步逻辑引用（AI 可读）」块；可选「本步逻辑引用（AI 可读）」块格式与示例见 [00_系统规则_通用项目协议 §8.4d](../00_系统规则_通用项目协议.md#84d-dna-为核心实践目标与核心指令引用-ai-可读)。

**核心指令块内引用 — 错误 vs 正确示例**（模板强制：禁止左侧写法，采用右侧写法）：

| 类型 | 错误示例（禁止） | 正确示例（必须） |
|------|------------------|------------------|
| 文档 | 必读：04_全链路…、02_三位一体、11_规约 | 03_原子目标与规约/_共享规约/04_全链路通信协议矩阵.md、03_原子目标与规约/_共享规约/02_三位一体仓库规约.md、03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md |
| DNA | dna_module_a.yaml、见 DNA | 03_原子目标与规约/_System_DNA/core_modules/dna_module_a.yaml 或 DNA 键路径（如 dna_dev_workflow.workflow_stages[stage1_01]） |
| L5 | 02_验收标准对应行 | 05_成功标识与验证/02_验收标准.md#l5-stage-stage1_01 |

**示例**：[Stage1-01 三位一体仓库初始化](Stage1_仓库与骨架/01_三位一体仓库_实践.md)、[Stage2-02 采集逻辑与Dockerfile](Stage2_数据采集与存储/02_A轨_采集逻辑与Dockerfile_实践.md) 为核心指令由 DNA 键展开的参考写法。

### 实践步骤验证与准出参考（Stage2-01 案例）

**其它所有实践步骤**宜参考 [Stage2-01 基础设施与依赖部署](Stage2_数据采集与存储/01_A轨_基础设施与依赖_实践.md) 的以下约定，保持可执行、可评判、下游可追溯：

| 约定 | 要求 | 参考位置 |
|------|------|----------|
| **版本固定与缓存** | Chart、中间件等部署文件必须版本固定并缓存（如 charts/dependencies/ 本地存放）；禁止部署时从远程动态拉取未固定版本；code 逻辑视情况固定 | [01_A轨_基础设施与依赖_实践#版本固定与缓存约定](Stage2_数据采集与存储/01_A轨_基础设施与依赖_实践.md#l4-stage2-01-version-pinning)、DNA `pinned_versions` |
| **验证三项** | ① 列出验证项 ② 列出验证方式与预期目标 ③ 评判验证结果是否符合预期（符合/不符合）；全部符合方可准出 | [01_A轨_基础设施与依赖_实践#验证与准出](Stage2_数据采集与存储/01_A轨_基础设施与依赖_实践.md#l4-stage2-01-exit) |
| **下游关键引用与核心验证** | 若有下游依赖，须有关键下游引用示例（配置如何添加、如何调用本步产出）；示例须可执行，且作为本步准出核心验证指标之一；全部验证通过才认为本步验证通过 | [01_A轨_基础设施与依赖_实践#关键下游引用与验证要求](Stage2_数据采集与存储/01_A轨_基础设施与依赖_实践.md#l4-stage2-01-downstream)；下游步骤文档须含「关键下游依赖」并注明验证归属 |
