...
请先阅读 [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md)、global_const.production_requirements.observability、本步骤「实施内容」。任务：1. 落地结构化日志与 log_level；2. 暴露关键指标（Prometheus 或等价）；3. 实现 /health 或心跳端点；4. 可验证（curl /health、Prometheus 抓取）。工作目录 diting-core / diting-infra。请输出：变更列表、关键配置摘录、验证命令与结果摘要。
