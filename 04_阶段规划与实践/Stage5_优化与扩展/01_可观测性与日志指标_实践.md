...
> - **战略维度**: [04_可观测性与运行保障](../../02_战略维度/平台与产品/04_可观测性与运行保障.md)
> - **原子能力**: [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md)、[09_核心模块架构规约](../../03_原子目标与规约/_共享规约/09_核心模块架构规约.md)（指标与日志）
> - **阶段**: [Phase3_优化与扩展](README.md)
> - **本步骤**: 日志/指标/健康检查满足 L3 production_requirements.observability

**本步设计文档**：[01_可观测性与日志指标设计](../../03_原子目标与规约/Stage5_优化与扩展/01_可观测性与日志指标设计.md#design-stage5-01-exit)  
**本步 DNA 文件**：[03_原子目标与规约/_System_DNA/Stage5_优化与扩展/dna_stage5_01.yaml](../../03_原子目标与规约/_System_DNA/Stage5_优化与扩展/dna_stage5_01.yaml)

<a id="l4-step-nav"></a>
## 步骤导航
- **上一步**：[03_回测或仿真验证](../Stage4_MoE与执行网关/03_评测或仿真验证_实践.md#l4-stage4-03-goal)
- **下一步**：[02_成本治理与Token熔断](02_成本治理与Token熔断_实践.md#l4-stage5-02-goal)
...
<a id="l4-stage5-01-goal"></a>
## 步骤目标

- **L3 阶段目标**：优化与扩展阶段满足生产级可观测性；日志、指标与健康检查与 [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md) 及 `global_const.production_requirements.observability` 一致。
...
3. [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md)、DNA observability 已定稿。
...
请先阅读 03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md、global_const.production_requirements.observability、本步骤「实施内容」。任务：1. 落地结构化日志与 log_level；2. 暴露关键指标（Prometheus 或等价）；3. 实现 /health 或心跳端点；4. 可验证（curl /health、Prometheus 抓取）。工作目录 diting-core / diting-infra。请输出：变更列表、关键配置摘录、验证命令与结果摘要。
...
1. [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md)
2. DNA production_requirements.observability；本步骤「实施内容」「验收与测试」

## 依赖的 Stage

- 依赖 [Stage2_数据采集与存储](../Stage2_数据采集与存储/README.md)、[Stage3_模块实践](../Stage3_模块实践/README.md) 进行中或准出。
...
3. **健康检查与心跳**
   - 按 [08_心跳协议与健康检查规约](../../03_原子目标与规约/_共享规约/08_心跳协议与健康检查规约.md)：关键组件心跳上报或 /health 端点；超时熔断策略可配置或占位。
...
- [ ] 已更新 L5 [02_验收标准](../../05_成功标识与验证/02_验收标准.md) 中**功能验收表**本步对应行及 [l5-stage-stage5_01](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage5_01)（状态与验证方式）
...
- **本步准出即 Stage s3 准出条件之一**；本步验收命令纳入 [Stage3_模块实践 01_](../Stage3_模块实践/01_语义分类器_实践.md#l4-stage3-01-exit)、[Stage4_MoE与执行网关 01_](../Stage4_MoE与执行网关/01_ModuleC_MoE议会接入.md#l4-stage4-01-exit) 可执行验证清单。
- **本步产出**：供 Stage3/Stage4 01_ 可观测性验收使用；环境观测/灰度/回滚依赖本步指标与健康检查。
- **本步依赖**：依赖 [Stage2](../Stage2_数据采集与存储/README.md)、[Stage3](../Stage3_模块实践/README.md) 进行中或准出；依赖 Stage1、Stage2/3 核心链路就绪。
...
| `dna_stage5_01.semantic_refs`、`global_const.heartbeat_protocol` | 心跳与健康检查规约（见本步 DNA） |
