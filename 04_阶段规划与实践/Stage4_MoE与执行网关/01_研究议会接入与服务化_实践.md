...
**本步设计文档**：[01_研究议会接入与服务化_设计](../../03_原子目标与规约/Stage4_MoE与执行网关/01_研究议会接入与服务化_设计.md#design-stage4-01-exit)  
...
- **下一步**：[02_外部动作边界接入_实践](02_外部动作边界接入_实践.md#l4-stage4-02-goal)
...
- **L3 阶段目标**：MoE（混合专家）议会与执行网关接入；Module C 根据 Domain Tag 路由到对应专家，专家意见与 **Module D 决策中枢**投票链路对接，形成 Alpha = (Quant ∩ Router) × Kelly 的完整链路。
...
| **DON'T** | 不要改变 D（决策中枢）已约定的 ExpertOpinion 结构（可扩展字段如 `horizon`，见 expert.proto） |
| **可选** | 专家池可包含 VC-Agent（信仰专家），输出 `TimeHorizon = LONG_TERM` 的 ExpertOpinion，供决策中枢按期限分流；见 [09_ Module C VC-Agent](../../03_原子目标与规约/_共享规约/09_核心模块架构规约.md) |
| **`long_term_candidate` → VC-Agent** | Router 按 `quant_signal.long_term_candidate` 路由至 VC-Agent：`diting-core/diting/moe/router.py`（route_and_collect_opinions）、`vc_agent.py`（vc_agent_opinion 返回 horizon=LONG_TERM）；产品与范围见 [01_需求与产品范围](../../03_原子目标与规约/平台与产品/01_需求与产品范围.md)。输入契约、`b_track_candidate_snapshot`（表名保留为历史兼容）与 Segment 规约见 [_共享规约/11_ 数据采集与输入层规约](../../03_原子目标与规约/_共享规约/11_数据采集与输入层规约.md)、[_共享规约/12_右脑数据支撑与Segment规约](../../03_原子目标与规约/_共享规约/12_右脑数据支撑与Segment规约.md)；实践见 [03_信号扫描与候选生成_实践](../Stage3_模块实践/03_信号扫描与候选生成_实践.md)、[02_外部动作边界接入_实践](02_外部动作边界接入_实践.md) |
...
- Phase1 准出：D（决策中枢）可接收 Quant + Expert 意见并输出 Alpha。
- 工作目录 **diting-core**。
...
1. **Router**
   - 实现 MoE Router：输入 symbol、domain_tags（来自 Module A）、quant_signal（来自 Module B）；按领域标签分发到对应专家（农业/科技/宏观），未知 进 Trash Bin（见 09_ Module C）。当 `quant_signal.long_term_candidate` 为 True 且 enable_vc_agent 时，将请求路由至 VC-Agent（见 diting/moe/router.py、vc_agent.py）。
...
- **ExpertOpinion 结构**：与 09_、Phase1 D（决策中枢）输入一致（is_supported、direction、confidence、reasoning_summary）；C 输出 List[ExpertOpinion] 可直接作为 D 的 vote(quant_signal, expert_opinions) 输入。
...
完成本步且通过验收、DoD 全勾后，进入 [02_外部动作边界接入_实践](02_外部动作边界接入_实践.md)。**重要**：02_ 依赖 E 风控输出可流入执行层，C 未就绪不影响 02_ 占位，但完整链路需 01_ 准出。
...
- **本步准出即 Stage s2 准出条件之一**；本步验收命令纳入 [Stage2_数据采集与存储 01_](../Stage2_数据采集与存储/01_数据底座与基础依赖_实践.md#l4-stage2-01-exit) 可执行验证清单。
