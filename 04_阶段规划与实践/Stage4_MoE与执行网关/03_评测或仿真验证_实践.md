...
> - **战略维度**: [01_产品范围与优先级](../../02_战略维度/平台与产品/01_产品范围与优先级.md)、[06_评测一致性与发布闭环](../../02_战略维度/平台与产品/06_评测一致性与发布闭环.md)
> - **原子能力**: [01_需求与产品范围](../../03_原子目标与规约/平台与产品/01_需求与产品范围.md)、[09_核心模块架构规约](../../03_原子目标与规约/_共享规约/09_核心模块架构规约.md)
> - **阶段**: [Phase2_MoE与执行网关](README.md)
> - **本步骤**: Docker/K3s 下回测或仿真路径跑通，研产同构可验证

**本步设计文档**：[03_回测或仿真验证设计](../../03_原子目标与规约/Stage4_MoE与执行网关/03_评测或仿真验证_设计.md#design-stage4-03-exit)  
**本步 DNA 文件**：[03_原子目标与规约/_System_DNA/Stage4_MoE与执行网关/dna_stage4_03.yaml](../../03_原子目标与规约/_System_DNA/Stage4_MoE与执行网关/dna_stage4_03.yaml)  
**逻辑填充期接入点**：本步须按设计文档「逻辑填充期架构参照：Lean 四层解耦与仿真度」小节实现并达标，见 [design-stage4-03-integration-lean](../../03_原子目标与规约/Stage4_MoE与执行网关/03_评测或仿真验证_设计.md#design-stage4-03-integration-lean)。

<a id="l4-step-nav"></a>
## 步骤导航
- **上一步**：[02_ModuleF执行网关接入](02_ModuleF执行网关接入.md#l4-stage4-02-goal)
- **下一步**：[01_可观测性与日志指标](../Stage5_优化与扩展/01_可观测性与日志指标_实践.md#l4-stage5-01-goal)
...
2. [Stage2](../Stage2_数据采集与存储/README.md) 或 [Stage3](../Stage3_模块实践/README.md) 进行中；工作目录 **diting-core** / **diting-infra** 就绪。
...
请先阅读本步骤「实施内容」、设计文档「逻辑填充期架构参照：Lean 四层解耦与仿真度」小节、04_阶段规划与实践/Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-goal 或 04_阶段规划与实践/Stage3_模块实践/01_语义分类器_实践.md#l4-stage3-01-goal。任务：1. 在 Docker/K3s 下运行完整回测或仿真 A→F；2. 按设计文档 Lean 四层与仿真度要求实现并达标；3. 验证研产同构（同一套策略与 Broker 抽象）；4. 产出可判定结果（PnL 摘要、订单流日志）。工作目录 diting-core / diting-infra。请输出：执行命令、环境说明、结果摘要。
...
1. [Stage2 01_](../Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-exit) 或 [Stage3 01_](../Stage3_模块实践/01_ModuleA.md#l4-stage3-01-exit)
2. 本步骤「实施内容」「验收与测试」

## 依赖的 Stage

- 依赖 [Stage2_数据采集与存储](../Stage2_数据采集与存储/README.md)（s2）或 [Stage3_模块实践](../Stage3_模块实践/README.md)（s3）进行中；或 Stage4 前两步（01_、02_）已完成。
...
<a id="l4-stage4-03-exit"></a>
## 本步骤准出（DoD）

- [ ] 代码/配置已提交至约定分支（工作目录 diting-core / diting-infra）
- [ ] Docker 或 K3s 下回测/仿真任务可执行并成功结束，与 s2/s3 exit_criteria 一致
- [ ] Code Review 通过（或标注豁免）
- [ ] L3 Lean 四层解耦与仿真度按设计文档达标；已更新 L5 [02_验收标准](../../05_成功标识与验证/02_验收标准.md) 中**功能验收表**本 Phase/Stage2-3 对应行及 [l5-stage-stage4_03](../../05_成功标识与验证/02_验收标准.md#l5-stage-stage4_03)（状态与验证方式）
...
完成本步且通过验收、DoD 全勾后，Stage4 准出；可进入 [Stage5_优化与扩展](../Stage5_优化与扩展/README.md)。**重要**：与 Stage2/Stage3 准出条件对齐后再进入后续阶段。
...
- **本步准出即 Stage s2 或 s3 准出条件之一**；本步验收命令纳入 [Stage2 01_](../Stage2_数据采集与存储/01_基础设施与依赖_实践.md#l4-stage2-01-exit)、[Stage3 01_](../Stage3_模块实践/01_ModuleA.md#l4-stage3-01-exit) 可执行验证清单。
- **本步产出**：供 Stage2/Stage3 01_ 可执行验证清单使用；回测/仿真成功跑完为 s2/s3 准出依据之一。
- **本步依赖**：依赖 [Stage2](../Stage2_数据采集与存储/README.md)（s2）或 [Stage3](../Stage3_模块实践/README.md)（s3）进行中；依赖 Stage4 [01_](01_ModuleC_MoE议会接入.md#l4-stage4-01-exit)、[02_](02_ModuleF执行网关接入.md#l4-stage4-02-exit) 准出。
