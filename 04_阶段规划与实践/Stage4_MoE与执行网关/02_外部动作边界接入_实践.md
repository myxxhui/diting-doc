...
> - **战略维度**: [01_产品范围与优先级](../../02_战略维度/平台与产品/01_产品范围与优先级.md)、[08_外部系统接入与边界抽象](../../02_战略维度/平台与产品/08_外部系统接入与边界抽象.md)
...
**依赖与构建**：执行网关镜像的依赖与 [04_决策风控与动作主链_实践](../Stage3_模块实践/04_决策风控与动作主链_实践.md) 及 dna_module_f.integration_packages 一致；确保占位/仿真或实盘适配器在镜像内可运行。
