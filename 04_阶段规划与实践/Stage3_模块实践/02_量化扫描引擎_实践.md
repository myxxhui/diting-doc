...
下表为本步**全部验证项**：命令、工作目录、期望结果与**结果填写**合一。执行顺序按 [本步实践总结](#l4-stage3-02-summary) 步骤表；每完成一步即在对应行填写「测试结果」与「实际输出或备注」。**可执行命令**见 [可执行验证命令汇总](#可执行验证命令汇总)。
...
以下命令均在 **diting-core** 根目录执行（除「镜像内」外）；与 [验证项与结果清单](#l4-stage3-02-verify-checklist)、Makefile 对齐。AI 执行时按本表逐项运行并填写验证清单对应行。
...
| V-TEST | `make test` 或 `pytest diting/scanner -v --cov=diting.scanner`（或等价）；覆盖 TREND/REVERSION/BREAKOUT 及过滤逻辑 | 退出码 0；覆盖率 ≥ 80%（与 [02_验收标准](../../05_成功标识与验证/02_验收标准.md) 质量验收一致） |
| V-IMAGE | 构建 module_b 镜像后，`docker run ... diting-module-b:tag make test`（或等价）在**容器内**执行 | 退出码 0；依赖链在镜像内完整 |
| 一键构建 | `make build-module-b`（或 `make build-images` 含 module_b） | 退出码 0；镜像 diting-module-b 存在 |
